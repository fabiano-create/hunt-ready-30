...
export async function runVision({ creds, system, schema, content, maxTokens = 1024, effort = 'medium' }) {
  ...
  try {
    ...
    if (response.stop_reason === 'max_tokens') return { ok: false, status: 502, body: { error: 'The AI answer was cut off. Try again.', code: 'truncated' } };
    ...
    return { ok: true, data, model: response.model || usedModel, via: creds.mode };
  } catch (error) {
