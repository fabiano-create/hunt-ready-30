...
function exerciseDef(name,mode){ mode=mode||(active?.workout?.mode)||equipmentMode(); for(const w of Object.values(PROGRAM)) for(const e of w.exercises){ const x=substituteExercise(e,mode); if(x.name===name) return x; } for(const m of ['dumbbells','bodyweight']) for(const w of Object.values(PROGRAM)) for(const e of w.exercises){ const x=substituteExercise(e,m); if(x.name===name) return x; } return null; }
...
function sessionExercises(day,mode){ return PROGRAM[day].exercises.map(e=>substituteExercise(e,mode)).filter(e=>e.type!=='rest'); }
