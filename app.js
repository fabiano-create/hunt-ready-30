...
function openPastSession(){ const today=localDateKey(), now=new Date(); const wd=dayName(); openModal(`<div class="close-row"><div><div class="kicker">HISTORY</div><h2>Log a past session</h2><p class="note">For a workout you did but didn't log in the app, or to restore one that was lost.</p></div><button class="icon-btn" onclick="closeModal()">×</button></div>
  <div class="set-grid"><div class="field"><label>Date</label><input id="psDate" type="date" value="${today}" max="${today}" onchange="const d=new Date(this.value+'T12:00:00'); $('#psDay').value=d.toLocaleDateString('en-US',{weekday:'long'});"></div><div class="field"><label>Time</label><input id="psTime" type="time" value="${String(now.getHours()).padStart(2,'0')}:${String(Math.max(0,now.getMinutes()-30)).padStart(2,'0')}"></div></div>
  <div class="set-grid"><div class="field"><label>Workout</label><select id="psDay">${Object.keys(PROGRAM).map(d=>`<option value="${d}" ${d===wd?'selected':''}>${d} · ${PROGRAM[d].title}</option>`).join('')}</select></div><div class="field"><label>Duration (min)</label><input id="psDuration" inputmode="numeric" value="30"></div></div>
  <div class="field"><label>Equipment</label><select id="psMode"><option value="dumbbells" ${equipmentMode()==='dumbbells'?'selected':''}>Dumbbells</option><option value="bodyweight" ${equipmentMode()==='bodyweight'?'selected':''}>Bodyweight</option></select></div>
  <div class="field"><label>What you did (optional)</label><textarea id="psNotes" rows="3" placeholder="e.g. 3 rounds of the circuit, felt hard"></textarea></div>
  <p class="note">This logs the session and shows it as done on that day. Individual sets aren't recorded, so it won't move your progression charts.</p>
  <button class="primary" onclick="savePastSession()">LOG SESSION</button>`); }
function savePastSession(){ const date=$('#psDate').value, time=$('#psTime').value||'12:00', day=$('#psDay').value, duration=Math.max(1,+$('#psDuration').value||30), mode=$('#psMode').value, notes=$('#psNotes').value.trim(); if(!date){ alert('Pick a date.'); return; } const when=new Date(`${date}T${time}:00`); if(isNaN(when)){ alert('That date or time is not valid.'); return; } const h=getState().history; h.unshift({id:uid(),date:when.toISOString(),day,title:PROGRAM[day].title,duration,logs:[],summary:notes?`Logged afterward: ${notes}`:'Logged afterward.',mode,manual:true}); h.sort((a,b)=>new Date(b.date)-new Date(a.date)); STORE.set('hr30_history',h.slice(0,500)); closeModal(); trainView='history'; renderTrain(); }
function renderHistoryMarkup(){ const h=getState().history; return `<button class="secondary" style="width:100%;margin-bottom:12px" onclick="openPastSession()">＋ LOG A PAST SESSION</button><section class="card">${h.length?h.map(x=>`<div class="history-item"><div class="top"><div><h4>${x.day} — ${x.title}</h4><small>${new Date(x.date).toLocaleString()}</small></div><strong>${x.duration}m</strong></div><p class="note">${x.logs?.length||0} logged sets / blocks</p><button class="ghost" onclick="viewHistory('${x.id||''}','${x.date}')">View details →</button></div>`).join(''):`<div class="empty">Complete your first workout and it will show up here.</div>`}</section>`; }
