...
function exerciseDef(name,mode){ mode=mode||(active?.workout?.mode)||equipmentMode(); const cands=[]; for(const w of Object.values(PROGRAM)) for(const e of w.exercises){ const x=substituteExercise(e,mode); cands.push(x); if(mode!=='bodyweight') cands.push(...blockVariantsFor(x)); } let f=cands.find(x=>x.name===name); if(f) return f; for(const m of ['dumbbells','bodyweight']) for(const w of Object.values(PROGRAM)) for(const e of w.exercises){ const x=substituteExercise(e,m); if(x.name===name) return x; if(m==='dumbbells'){ const v=blockVariantsFor(x).find(y=>y.name===name); if(v) return v; } } return null; }
...
function applyBlock(e,block,mode){ if(mode==='bodyweight') return e; const v=BLOCK_VARIANTS[e.name]?.[block]; if(!v) return e; const x={...e,...v,blockOf:e.name,block}; if(x.type==='strength'&&v.min){ x.prescription=`${x.sets} × ${x.min}${x.max>x.min?'–'+x.max:''}${v.suffix||''}`; } return x; }
function blockVariantsFor(e){ return [2,3].map(b=>applyBlock(e,b,'dumbbells')).filter(x=>x!==e); }
...
function sessionSetsMarkup(day,mode,logs){
  ...
  return sessionExercises(day,mode).map((e,i)=>{ const g=groups[e.name]||[], first=g[0]||{}, n=g.length||'', id=`se${i}`;
    ...
    else if(e.type==='strength') inputs=`<div class="set-grid three">${num('Weight (lb)','weight',first.weight)}${num('Reps','reps',first.reps,e.min)}${num('Sets','sets',n,e.sets)}</div>`;
...
function openSessionEditor(id){
  ...
  <div class="field"><label>Equipment</label><select id="psMode" onchange="rerenderSessionSets()"><option value="dumbbells" ${mode==='dumbbells'?'selected':''}>Dumbbells</option><option value="bodyweight" ${mode==='bodyweight'?'selected':''}>Bodyweight</option></select></div>
