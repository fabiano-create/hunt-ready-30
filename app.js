...
function sessionSetsMarkup(day,mode,logs){
  ...
  return sessionExercises(day,mode).map((e,i)=>{ const g=groups[e.name]||[], first=g[0]||{}, n=g.length||'', id=`se${i}`;
    ...
    else if(e.type==='circuit') inputs=`<div class="set-grid">${num('Rounds','rounds',first.rounds)}${num('Minutes','minutes',first.minutes,e.minutes)}</div>`;
...
function collectSessionLogs(){ const logs=[]; document.querySelectorAll('#sessionSets .session-ex').forEach(box=>{ const name=box.dataset.name,type=box.dataset.type,id='se'+box.dataset.idx; const v=k=>{ const el=$(`#${id}-${k}`); return el?el.value:''; }; const n=k=>Number(v(k))||0;
  ...
  else if(type==='circuit'){ if(!(n('rounds')||n('minutes'))) return; logs.push({name,set:1,type,rounds:n('rounds'),minutes:n('minutes')}); }
...
function updateWorkingWeightsFromLogs(logs){ const w=getState().workingWeights;logs.forEach(l=>{ if(l.level) w[l.name+'::level']=l.level; else if(l.band) w[l.name]=l.band; else if(['strength','carry'].includes(l.type)&&l.weight) w[l.name]=l.weight; });STORE.set('hr30_workingWeights',w); }
