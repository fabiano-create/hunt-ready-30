...
const PROGRAM = {
  ...
  Friday:{title:"Hunter Conditioning",focus:"Work capacity • controlled intensity",duration:30,exercises:[
    ...
    {name:"20-Min Hunter Circuit",prescription:"20 min quality rounds",type:"circuit",minutes:20,rest:0,notes:"8 goblet squats • 8 push-ups • 10 DB RDLs • 8 reverse lunges/leg • 10 one-arm rows/side • 30-sec farmer carry. Do not race.",items:[{name:"Goblet Squat",reps:"8",load:true},{name:"Push-Up",reps:"8"},{name:"DB RDL",reps:"10",load:true},{name:"Reverse Lunge",reps:"8/leg",load:true,optional:true},{name:"1-Arm DB Row",reps:"10/side",load:true},{name:"Farmer Carry",reps:"30 sec",load:true}]},
...
const BENCHMARKS_GYM = [
  {id:"trapbar",label:"Trap-bar deadlift",target:225,unit:"lb × 5"},
  {id:"backsquat",label:"Back squat",target:185,unit:"lb × 5"},
  {id:"bench",label:"Bench press",target:155,unit:"lb × 5"},
  {id:"pullups",label:"Pull-ups",target:8,unit:"clean reps (or pulldown at body weight × 8)"},
  {id:"handles",label:"Farmer handles",target:200,unit:"lb total × 60 sec"},
  {id:"plank",label:"Plank",target:90,unit:"seconds"},
  {id:"stairs",label:"Stair climber",target:30,unit:"lb pack for 30 min"}
];
const BENCHMARKS = [
...
const EXERCISE_GUIDES = {
  ...
  "Split Squat": {
    subtitle: "Block 2 squat. One leg does most of the work, both feet stay planted.",
    steps: ["Take a long stride: front foot flat, back foot on its toes.", "Hold dumbbells at your sides or one at your chest.", "Lower straight down until the back knee hovers just off the floor.", "Drive through the front foot to stand. Finish all reps, then switch legs."],
    cues: ["Torso tall, slight forward lean is fine.", "Front knee tracks over the toes.", "Down like an elevator, not a swing."],
    mistakes: ["Stance too short, so the front heel lifts.", "Letting the front knee cave inward.", "Bouncing off the back knee."],
    why: "Hunting is one leg at a time: side-hills, deadfall, climbing into a stand. This builds that strength and balance without a heavy load on your spine."
  },
  "Bulgarian Split Squat": {
    subtitle: "Block 3 squat. The split squat with the rear foot on a bench or step.",
    steps: ["Stand a stride's length in front of a knee-high bench or step.", "Put the top of your back foot on it, laces down.", "Lower until the front thigh is near parallel and the back knee dips low.", "Push through the whole front foot to stand. Switch legs after the set."],
    cues: ["Front shin near vertical.", "Weight through mid-foot and heel.", "Breathe out on the way up."],
    mistakes: ["Bench too high, which arches the back.", "Leaning way forward to make it easier.", "Rushing the descent."],
    why: "The strongest single-leg movement you can do with dumbbells. It makes packing out uphill feel like walking."
  },
  "Staggered-Stance RDL": {
    subtitle: "Block 2 hinge. One foot slightly back so one hamstring does most of the work.",
    steps: ["Stand tall, then slide one foot back so its toes sit beside the front heel, heel up.", "Hold dumbbells in front of your thighs.", "Push the front hip back; let the dumbbells slide down the front leg.", "Stop at a strong hamstring stretch with a flat back, then drive the hip forward to stand."],
    cues: ["Most of your weight on the front foot.", "Back toe is a kickstand for balance, not a driver.", "Flat back, eyes a few feet ahead."],
    mistakes: ["Twisting the hips toward the back leg.", "Rounding the low back.", "Bending the front knee into a squat."],
    why: "It teaches the single-leg hinge with a safety net, and it exposes the side that is weaker so you can fix it."
  },
  "Single-Leg RDL": {
    subtitle: "Block 3 hinge. Balance, hamstring strength, and a stable hip on one leg.",
    steps: ["Stand on one leg, soft knee, dumbbell in the opposite hand (or both hands).", "Hinge at the hip and let the free leg float straight back as your chest comes forward.", "Reach the dumbbell toward the floor, stopping when the back is still flat.", "Squeeze the standing glute to return. Finish the set, then switch legs."],
    cues: ["Hips square to the floor; imagine headlights on your pelvis pointing down.", "Free leg and torso move as one lever.", "Go slow; balance is the exercise."],
    mistakes: ["Opening the hip so the free leg swings out.", "Rounding the back to reach lower.", "Locking the standing knee."],
    why: "Every stalk is a series of single-leg balances under load. This is the strength and control behind quiet, sure-footed movement."
  },
  "Single-Arm Floor Press": {
    subtitle: "Block 3 press. Same floor press, one dumbbell, so your core has to fight the tilt.",
    steps: ["Lie on the floor, knees bent, one dumbbell pressed over your chest.", "Brace as if someone is about to push on your ribs; free hand on your stomach.", "Lower until the upper arm touches the floor, elbow about 45° from your side.", "Press back up without letting your hips or shoulders rotate. Switch sides after the set."],
    cues: ["Wrist stacked over elbow.", "Ribs down, hips still.", "The dumbbell path is slightly diagonal, toward the opposite shoulder."],
    mistakes: ["Letting the body roll toward the weight.", "Bouncing the elbow off the floor.", "Flaring the elbow straight out."],
    why: "Pressing with one arm is how you push a pack up a ledge or brace on a rock. It builds pressing strength and the trunk stiffness that keeps a bow arm steady."
  },
  "Gorilla Row": {
    subtitle: "Block 2 row. Two dumbbells on the floor, hinge over them, and row one at a time.",
    steps: ["Set two dumbbells on the floor between your feet, feet a little wider than shoulders.", "Hinge at the hips with a flat back until you can hold both handles.", "Row one dumbbell to your hip while the other stays on the floor as a brace.", "Lower under control and row the other side. Alternate for the set."],
    cues: ["Flat back the whole set; the non-working arm keeps you honest.", "Elbow to the hip, not out to the side.", "Pause at the top for a beat."],
    mistakes: ["Standing up between reps.", "Rotating the torso to yank the weight.", "Rounding the back as you tire."],
    why: "It's a row and a loaded hinge hold at once, which is exactly the posture you're in when dragging a deer."
  },
  "Renegade Row": {
    subtitle: "Block 3 row. A plank on two dumbbells, rowing one at a time.",
    steps: ["Set two dumbbells shoulder-width apart and take a push-up position with a hand on each.", "Widen your feet for a stable base.", "Row one dumbbell to your ribs while the other arm and both legs hold the plank.", "Set it down quietly and row the other side."],
    cues: ["Hips level; don't let them rotate toward the rowing arm.", "Squeeze the glutes and brace hard.", "Slow and controlled beats fast and sloppy."],
    mistakes: ["Hips twisting open on every rep.", "Sagging low back.", "Using dumbbells that roll; hex dumbbells are safest."],
    why: "Total-body tension with a row on top. It builds the anti-rotation strength that keeps your shot steady from an awkward position."
  },
  "Rack Carry": {
    subtitle: "Block 3 carry. Dumbbells held at the shoulders, like a pack on your chest.",
    steps: ["Clean two dumbbells to your shoulders, one end resting on each shoulder.", "Elbows forward and slightly up, ribs down, brace.", "Walk slow, short steps for the timed interval.", "Set them down under control; don't drop them."],
    cues: ["Tall through the crown of the head.", "Breathe behind the brace; don't hold your breath the whole carry.", "Eyes forward, not down."],
    mistakes: ["Leaning back to hold the weight up.", "Letting the elbows drop so the weight pulls you forward.", "Going so heavy that posture collapses."],
    why: "Weight high on the body is the hardest carry for your trunk and the most like a loaded pack. This is the last piece of pack-out strength."
  },
  "Back Squat": {
    subtitle: "Gym Block 1 squat. The bar on your upper back, inside a rack.",
    steps: ["Set the bar at collarbone height, safeties just below your bottom position.", "Bar across the upper back, hands just outside the shoulders, elbows down.", "Stand up, take two steps back, feet shoulder width, toes slightly out.", "Big breath, brace, sit down between the hips until thighs are at least parallel.", "Drive the floor away and stand. Rack it with two steps forward."],
    cues: ["Chest up, knees over toes.", "Whole foot flat; push through the middle of the foot.", "Same walk-out every time."],
    mistakes: ["Safeties set too low to matter.", "Bouncing out of the bottom.", "Knees caving as you tire."],
    why: "Nothing builds pack-out legs faster. It also teaches you to brace, which protects your back under a loaded pack."
  },
  "Front Squat": {
    subtitle: "Gym Block 2 squat. Bar on the front of the shoulders, torso upright.",
    steps: ["Set the bar at collarbone height. Take a clean grip or cross your arms; elbows high either way.", "Bar rests on the shoulders, not the hands.", "Unrack, two steps back, feet shoulder width.", "Brace and sit straight down, elbows high the whole way.", "Stand tall and rack it."],
    cues: ["Elbows up, or the bar rolls forward.", "Torso as vertical as you can keep it.", "Lighter than your back squat is normal."],
    mistakes: ["Letting the elbows drop at the bottom.", "Wrist pain from gripping the bar hard; let it rest on the shoulders.", "Rushing the descent."],
    why: "It forces an upright trunk and a strong upper back, the posture you want with a pack on and a rifle in your hands."
  },
  "Trap-Bar Deadlift": {
    subtitle: "Gym Block 1 hinge. The friendliest way to pull heavy off the floor.",
    steps: ["Stand in the middle of the trap bar, feet hip width.", "Hinge down and grab the handles at mid-foot.", "Flat back, chest up, shoulders over the handles.", "Push the floor away and stand tall; lock hips and knees together.", "Hinge back down under control and set it down."],
    cues: ["Big breath and brace before every pull.", "Bar stays close; you stand up with it, not around it.", "Squeeze the glutes at the top; don't lean back."],
    mistakes: ["Rounding the low back to get to the handles.", "Jerking the bar off the floor.", "Dropping the hips too low and turning it into a squat."],
    why: "The heaviest safe pull most hunters will ever do. It is the deer drag, the pack lift, and the cooler carry in one movement."
  },
  "Barbell RDL": {
    subtitle: "Gym Block 2 hinge. Same movement as the dumbbell RDL, more load.",
    steps: ["Take the bar from a rack at hip height or deadlift it to standing.", "Soft knees, bar against the thighs.", "Push the hips back and let the bar slide down the legs.", "Stop at a strong hamstring stretch with the back still flat, mid-shin at most.", "Drive the hips forward to stand."],
    cues: ["Bar drags the legs the whole way.", "Hips back, not down.", "Neck neutral; eyes a few feet ahead on the floor."],
    mistakes: ["Reaching for the floor by rounding.", "Bending the knees into a squat.", "Bouncing at the bottom."],
    why: "Hamstrings and glutes that can handle a long descent are what save your knees on the way down a mountain."
  },
  "Barbell Bench Press": {
    subtitle: "Gym Block 1 press. Set the safeties first, every time.",
    steps: ["Lie with your eyes under the bar, feet flat on the floor.", "Pull the shoulder blades back and down; a slight arch in the low back is fine.", "Grip just wider than the shoulders, unrack, and settle the bar over the shoulders.", "Lower to the lower chest with elbows about 45° from the body.", "Press back up to lockout over the shoulders."],
    cues: ["Wrists stacked over elbows.", "Drive the feet into the floor.", "Touch the chest, don't bounce off it."],
    mistakes: ["No safeties and no spotter.", "Flaring the elbows straight out.", "Lifting the hips off the bench."],
    why: "Pressing strength for pushing a pack up onto a rock, and the upper-body base that keeps a bow or rifle steady."
  },
  "Incline DB Bench Press": {
    subtitle: "Gym Block 2 press. Dumbbells on an incline bench, about 30°.",
    steps: ["Set the bench to a low incline, one or two notches up.", "Sit with dumbbells on your thighs, then kick them up as you lie back.", "Start with the dumbbells over the upper chest, palms forward.", "Lower until the elbows are level with the shoulders.", "Press up and slightly together."],
    cues: ["Shoulder blades pinned back.", "Elbows under the wrists.", "Control the lowering; the dumbbells don't have to touch."],
    mistakes: ["Bench set too steep, which turns it into a shoulder press.", "Dropping the dumbbells at the end instead of walking them back to the thighs.", "Going too heavy to kick up safely."],
    why: "Dumbbells make each arm work alone, which fixes the imbalance a bow arm or a rifle stock builds over a season."
  },
  "Overhead Press": {
    subtitle: "Gym Block 3 press. Standing, bar from the shoulders to lockout.",
    steps: ["Take the bar from a rack at shoulder height, grip just outside the shoulders.", "Stand with feet hip width, glutes and abs braced.", "Press the bar straight up, moving your head back slightly to clear it.", "Finish with the bar over the middle of your foot, arms locked.", "Lower it to the shoulders under control."],
    cues: ["Squeeze the glutes so the low back doesn't arch.", "Bar path is straight up, not out in front.", "Head through at the top, not before."],
    mistakes: ["Leaning back to press it up.", "Flaring the elbows out wide.", "Pressing from a bent-wrist grip."],
    why: "Whole-body stiffness under a load overhead is rare strength, and it's what hoisting a quarter onto a pack frame feels like."
  },
  "Cable Row": {
    subtitle: "Gym Block 1 row. Seated, cable at chest height, back tall.",
    steps: ["Sit with feet on the plate, knees slightly bent.", "Grab the handle and sit tall, shoulders down.", "Pull the handle to the lower ribs, elbows close to the body.", "Squeeze the shoulder blades together for a beat.", "Let the arms straighten and the shoulder blades reach forward under control."],
    cues: ["Torso stays still; the arms and back move.", "Lead with the elbows.", "Full stretch, full squeeze."],
    mistakes: ["Rocking the torso to swing the weight.", "Shrugging the shoulders up.", "Cutting the range short at either end."],
    why: "Upper-back strength you can control rep by rep. It supports posture under a pack and the draw of a bow."
  },
  "Chest-Supported Row": {
    subtitle: "Gym Block 2 row. Lying face down on an incline bench so your back can't cheat.",
    steps: ["Set a bench to about 30–45° and lie chest-down on it.", "Let the dumbbells hang straight down, arms long.", "Row the dumbbells to your hips, elbows back and in.", "Pause with the shoulder blades squeezed.", "Lower slowly to a full stretch."],
    cues: ["Chest stays on the pad the whole set.", "Elbows toward the back pockets.", "Slow on the way down."],
    mistakes: ["Lifting the chest off the bench to heave.", "Rowing to the shoulders instead of the hips.", "Using a weight you can only swing."],
    why: "Takes the low back out so the upper back does all the work, which is what you want the week after a heavy trap-bar block."
  },
  "Lat Pulldown": {
    subtitle: "Gym Block 3 row. Weighted pull-ups if you can do eight clean; otherwise the pulldown.",
    steps: ["Set the thigh pad so your legs are locked in.", "Grip just wider than the shoulders, palms forward.", "Lean back a few degrees, chest up.", "Pull the bar to the upper chest, elbows down and back.", "Let it rise under control until the arms are long."],
    cues: ["Pull with the elbows, not the hands.", "Chest up to meet the bar.", "Don't lean back further to finish the rep."],
    mistakes: ["Pulling behind the neck.", "Rocking back to use body weight.", "Half reps at the top."],
    why: "Vertical pulling builds the lats that carry a pack's weight on your shoulders and hips instead of your neck."
  },
  "Face Pull": {
    subtitle: "Rear shoulders and upper back, done at a cable with a rope.",
    steps: ["Set the cable at face height with a rope attachment.", "Grip the rope with thumbs pointing back toward you.", "Step back until the weight lifts, feet staggered.", "Pull the rope toward your face, elbows high and wide, hands finishing beside the ears.", "Return slowly."],
    cues: ["Elbows lead and stay high.", "Squeeze between the shoulder blades.", "Light weight, perfect reps."],
    mistakes: ["Using so much weight the torso rocks.", "Pulling to the chest instead of the face.", "Rushing the return."],
    why: "The antidote to hours of hunching over a bow, a rifle, or a steering wheel. Healthy shoulders make a steady shot."
  },
  "EZ-Bar Curl": {
    subtitle: "Arm and grip strength with the angled bar that's easier on the wrists.",
    steps: ["Grip the EZ bar on the angled sections, palms up.", "Stand tall, elbows at your sides.", "Curl the bar until the forearms are just past vertical.", "Lower under control to straight arms."],
    cues: ["Elbows pinned; only the forearms move.", "No swing from the hips.", "Squeeze at the top."],
    mistakes: ["Rocking the body to start the rep.", "Elbows drifting forward.", "Dropping the bar instead of lowering it."],
    why: "Biceps and forearms are what hold a pack strap, a drag rope, and a bow at full draw."
  },
  "Calf Raise Machine": {
    subtitle: "Standing calf machine, or a leg press for the same movement.",
    steps: ["Shoulders under the pads, balls of the feet on the edge of the step.", "Lower the heels below the step until you feel a stretch.", "Rise up as high as you can and pause.", "Lower slowly."],
    cues: ["Full range both directions.", "Pause at the top for a beat.", "Knees straight but not locked hard."],
    mistakes: ["Bouncing out of the bottom.", "Short, fast reps.", "Bending the knees to help."],
    why: "Calves and ankles take every step of a side-hill. Strong ones are the difference between sore and injured."
  },
  "Trap-Bar Carry": {
    subtitle: "Gym Block 2 carry. Pick the trap bar up and walk.",
    steps: ["Load the trap bar and stand inside it.", "Deadlift it to standing with a flat back.", "Walk short, quick steps for the timed interval.", "Set it down with the same hinge you picked it up with."],
    cues: ["Tall posture, ribs over hips.", "Squeeze the handles hard.", "Turn around slowly; the bar swings."],
    mistakes: ["Setting it down by rounding the back.", "Long, lunging steps.", "Going so heavy the posture folds."],
    why: "More load than you can hold in two dumbbells. This is the pack-out weight."
  },
  "Sandbag Carry": {
    subtitle: "Gym Block 3 carry. A sandbag hugged to the chest or on one shoulder.",
    steps: ["Hinge down and pull the bag up to your lap, then hug it to your chest (or heave it onto one shoulder).", "Stand tall and brace; the bag will shift.", "Walk for the timed interval; switch shoulders halfway if carrying on one.", "Set it down with a hinge, not a drop."],
    cues: ["Hug it tight; a loose bag is a heavy bag.", "Breathe behind the brace.", "Eyes forward."],
    mistakes: ["Leaning back to hold it.", "Letting it slide down to the belly.", "Dropping it from standing height."],
    why: "Nothing in a gym feels more like a quarter of an elk. The shifting weight trains the small stabilizers a barbell can't."
  },
  "Incline Treadmill Ruck": {
    subtitle: "Ruck day, indoors. A loaded pack on an incline treadmill.",
    steps: ["Put your pack on and cinch the straps so it rides high.", "Set the incline to 8–12% and the speed to a brisk walk, 3.0–3.5 mph.", "Walk for the full time without touching the rails.", "Last five minutes: drop the incline to flat and cool down."],
    cues: ["Tall posture; don't let the pack pull you back.", "Short quick steps.", "Nasal breathing if you can hold it."],
    mistakes: ["Holding the rails, which erases most of the work.", "Running.", "Starting with too much weight in the pack."],
    why: "Hills with a load are the single most specific thing you can do for a hunt. A treadmill lets you get them in on a weeknight."
  },
  "Stair Climber Ruck": {
    subtitle: "Gym Block 2 ruck. The stair climber with a loaded pack.",
    steps: ["Pack on, straps tight.", "Pick a step rate you can hold for the whole time; steady beats fast.", "Full foot on each step, hands off the rails.", "Last three minutes: slow down and cool off."],
    cues: ["Drive through the whole foot, not the toes.", "Stand tall.", "If you have to grab the rails, slow the machine down."],
    mistakes: ["Leaning on the rails.", "Bouncing on the toes.", "Sprinting the first five minutes and dying."],
    why: "Every step is a loaded step-up. Closest thing indoors to climbing to a stand with a pack on."
  },
...
const DEMO_VIDEOS = {
  ...
  "Split Squat":             {id:"la0pLPq-3A8", channel:"Get Exercise Confident", length:"3:17"},
  "Bulgarian Split Squat":   {id:"DeCnHqrN22U", channel:"Buff Dudes Workouts", length:"1:31"},
  "Staggered-Stance RDL":    {id:"Th4QdA2BOyg", channel:"J2FIT Strength & Conditioning", length:"1:02"},
  "Single-Leg RDL":          {id:"X2BM5J3UkEo", channel:"Ben Mighty", length:"1:05"},
  "Single-Arm Floor Press":  {id:"NgOG-2NtAtM", channel:"Littauer Strength Training", length:"1:06"},
  "Gorilla Row":             {id:"adYPUyioSHg", channel:"Berserkers and Barbells", length:"1:20"},
  "Renegade Row":            {id:"wTqlJ0aoJlM", channel:"BuiltLean", length:"1:02"},
  "Rack Carry":              {id:"KpDSmvbk0aw", channel:"Chris Bullock", length:"1:23"},
  "Back Squat":              {id:"aOzrA4FgnM0", channel:"Max Euceda (2-minute tutorial)", length:"2:01"},
  "Front Squat":             {id:"nmUof3vszxM", channel:"Torokhtiy", length:"2:32"},
  "Trap-Bar Deadlift":       {id:"a7sQPZsmxvA", channel:"Seriously Strong Training", length:"1:59"},
  "Barbell RDL":             {id:"3VXmecChYYM", channel:"Max Euceda (2-minute tutorial)", length:"2:00"},
  "Barbell Bench Press":     {id:"lWFknlOTbyM", channel:"Max Euceda (2-minute tutorial)", length:"2:00"},
  "Incline DB Bench Press":  {id:"IP4oeKh1Sd4", channel:"Max Euceda (2-minute tutorial)", length:"2:00"},
  "Overhead Press":          {id:"KP1sYz2VICk", channel:"Max Euceda (2-minute tutorial)", length:"2:00"},
  "Cable Row":               {id:"vwHG9Jfu4sw", channel:"Max Euceda (2-minute tutorial)", length:"2:00"},
  "Chest-Supported Row":     {id:"vmX58YYK3-8", channel:"Seriously Strong Training", length:"1:05"},
  "Lat Pulldown":            {id:"Z_3xHwuO8Tk", channel:"Seriously Strong Training", length:"1:53"},
  "Face Pull":               {id:"eTCBSFlCJ_s", channel:"NASM", length:"0:26"},
  "Sandbag Carry":           {id:"yu_i4ClKHlU", channel:"Core Blend Training", length:"0:31"},
  "Incline Treadmill Ruck":  {id:"kXIy_Ezhsbg", channel:"Jack Hanrahan Fitness", length:"11:20"},
...
const LEARN_ORDER = [
  "Hip Hinge","Goblet Squat","DB Romanian Deadlift","Reverse Lunge","1-Arm DB Row","DB Floor Press","Farmer Carry","Suitcase Carry","Step-Up","Plank","Push-Ups","Bent-Over Rear-Delt Raise","Hammer Curl","Standing Calf Raise","Weighted Walk / Ruck","Split Squat","Bulgarian Split Squat","Staggered-Stance RDL","Single-Leg RDL","Single-Arm Floor Press","Gorilla Row","Renegade Row","Rack Carry","Back Squat","Front Squat","Trap-Bar Deadlift","Barbell RDL","Barbell Bench Press","Incline DB Bench Press","Overhead Press","Cable Row","Chest-Supported Row","Lat Pulldown","Face Pull","EZ-Bar Curl","Calf Raise Machine","Trap-Bar Carry","Sandbag Carry","Incline Treadmill Ruck","Stair Climber Ruck","Shoulder Prep","Warm-up","Mobility",
...
function migrateLegacy(){
  ...
  if(!localStorage.hr30_settings) STORE.set('hr30_settings',{aiEndpoint:DEFAULT_AI_ENDPOINT,restSound:true,restVibrate:true});
...
function getState(){
  return {
    ...
    rifleProfile: STORE.get('hr30_rifleProfile',{}),
...
const EQUIPMENT_SUBS = {
  ...
  "20-Min Hunter Circuit":   { bodyweight:{name:"20-Min Hunter Circuit",type:"circuit",notes:"8 squats • 8 push-ups • 10 single-leg RDL/leg • 8 reverse lunges/leg • 10 table rows • 30-sec backpack carry. Do not race.",items:[{name:"Squat",reps:"8"},{name:"Push-Up",reps:"8"},{name:"Single-Leg RDL",reps:"10/leg"},{name:"Reverse Lunge",reps:"8/leg",load:true,optional:true},{name:"Table Row",reps:"10"},{name:"Backpack Carry",reps:"30 sec",load:true}]} }
};
...
function modeLabel(mode){ return mode==='bodyweight'?('Bodyweight'+(getState().settings.hasBands?' + bands':'')):mode==='gym'?'Gym':'Dumbbells'; }
function setTodayOverride(mode){ if(!mode||mode===baseEquipment()) localStorage.removeItem('hr30_todayOverride'); else STORE.set('hr30_todayOverride',{date:localDateKey(),mode}); renderToday(); }
function substituteExercise(e, mode){
  if(mode==='gym'){ const v=GYM_SUBS[e.name]; if(!v) return e; const x={...e,...v,subFor:e.name}; if(x.type==='strength'&&v.min){ x.sets=e.sets; x.rest=e.rest; const suffix=v.suffix||''; x.prescription=`${x.sets} × ${x.min}${x.max>x.min?'–'+x.max:''}${suffix}`; } return x; }
...
function exerciseDef(name,mode){ mode=mode||(active?.workout?.mode)||equipmentMode(); const cands=[]; for(const w of Object.values(PROGRAM)) for(const e of w.exercises){ const x=substituteExercise(e,mode); cands.push(x); if(mode!=='bodyweight') cands.push(...blockVariantsFor(x,mode)); } let f=cands.find(x=>x.name===name); if(f) return f; for(const m of ['dumbbells','gym','bodyweight']) for(const w of Object.values(PROGRAM)) for(const e of w.exercises){ const x=substituteExercise(e,m); if(x.name===name) return x; if(m!=='bodyweight'){ const v=blockVariantsFor(x,m).find(y=>y.name===name); if(v) return v; } } return null; }
function wwKey(name,def){ return def?.ladder?name+'::level':name; }
const CIRCUIT_PREFIX='Circuit · ';
function circuitLoadKey(item){ return CIRCUIT_PREFIX+item; }
function circuitLoadedItems(e){ return (e?.items||[]).filter(i=>i.load); }
function suggestedCircuitLoad(circuitName,item){ for(const h of getState().history){ const f=(h.logs||[]).find(l=>l.name===circuitName&&l.loads&&l.loads[item]); if(f) return f.loads[item]; } const w=getState().workingWeights[circuitLoadKey(item)]; return typeof w==='number'&&w?w:''; }
function circuitLoadInputs(e,idPrefix,existing){ const items=circuitLoadedItems(e); if(!items.length) return ''; return `<div class="kicker" style="margin-top:14px">LOADS USED (LB)</div><div class="set-grid">${items.map((it,k)=>`<div class="field"><label>${esc(it.name)} · ${esc(it.reps)}${it.optional?' · opt.':''}</label><input id="${idPrefix}${k}" data-item="${esc(it.name)}" inputmode="decimal" value="${existing?(existing[it.name]??''):suggestedCircuitLoad(e.name,it.name)}" placeholder="${it.optional?'0 = bodyweight':'lb'}"></div>`).join('')}</div>`; }
function readCircuitLoads(idPrefix){ const loads={}; document.querySelectorAll(`input[id^="${idPrefix}"][data-item]`).forEach(inp=>{ const v=Number(inp.value)||0; if(v>0) loads[inp.dataset.item]=v; }); return loads; }
...
function planLabel(){ const p=huntPlan(); const b=currentBlock(); return p?`${p.label} • week ${p.phaseWeek} of ${p.phaseWeeks} • Block ${b}`:`Week ${weekNumber()} • Block ${b}`; }
// ---- Training blocks: same slots, different movement every phase (dumbbell mode). Bodyweight mode progresses via ladders instead.
// ---- Gym mode: same slots, barbells, cables, and machines. Loads log as total bar weight (bar included).
const SAFETY={
  rack:"Squat inside a rack with the safety pins set just below your bottom position. If a rep stalls, sit down onto the pins. Don't fight it.",
  bench:"Never bench alone without safety pins set at chest height or a spotter. Leave the collars off when alone so you can tip plates off if you get stuck.",
  overhead:"Press in open floor space or a rack. If a rep stalls, lower the bar to your shoulders and rack it. Don't chase it overhead.",
  deadlift:"Set up with a flat back before every pull. If the bar drifts away from your shins, stop the set. Drop the weight rather than round to finish."
};
const GYM_SUBS={
  "Goblet Squat":{name:"Back Squat",barbell:true,safety:"rack",min:5,max:8},
  "DB Romanian Deadlift":{name:"Trap-Bar Deadlift",barbell:true,safety:"deadlift",min:5,max:8},
  "DB Floor Press":{name:"Barbell Bench Press",barbell:true,safety:"bench",min:5,max:8},
  "1-Arm DB Row":{name:"Cable Row",min:8,max:12},
  "Bent-Over Rear-Delt Raise":{name:"Face Pull",min:12,max:15},
  "Hammer Curl":{name:"EZ-Bar Curl",barbell:true,min:8,max:12},
  "Standing Calf Raise":{name:"Calf Raise Machine",min:12,max:15},
  "Farmer Carry":{notes:"Farmer handles if the gym has them; otherwise the heaviest dumbbells you can hold with a tall posture."},
  "Weighted Walk / Ruck":{name:"Incline Treadmill Ruck",prescription:"30 min",minutes:30,notes:"Pack on. 8–12% incline, 3.0–3.5 mph. Hands off the rails; that's the whole point."}
};
const GYM_BLOCK_VARIANTS={
  "Back Squat":{2:{name:"Front Squat",barbell:true,safety:"rack",min:5,max:8},3:{name:"Bulgarian Split Squat",barbell:false,safety:"",min:6,max:8,suffix:"/leg"}},
  "Trap-Bar Deadlift":{2:{name:"Barbell RDL",barbell:true,safety:"deadlift",min:6,max:10},3:{name:"Single-Leg RDL",barbell:false,safety:"",min:6,max:8,suffix:"/leg"}},
  "Barbell Bench Press":{2:{name:"Incline DB Bench Press",barbell:false,safety:"bench",min:8,max:10},3:{name:"Overhead Press",barbell:true,safety:"overhead",min:5,max:8}},
  "Cable Row":{2:{name:"Chest-Supported Row",min:8,max:12},3:{name:"Lat Pulldown",min:8,max:12}},
  "Farmer Carry":{2:{name:"Trap-Bar Carry",prescription:"4 × 40 sec",sets:4,notes:""},3:{name:"Sandbag Carry",prescription:"4 × 40 sec",sets:4,notes:""}},
  "Incline Treadmill Ruck":{2:{name:"Stair Climber Ruck",prescription:"35 min",minutes:35,notes:"Pack on, steady pace you can hold for the whole 35. Stand tall; don't lean on the machine."},3:{prescription:"40 min",minutes:40,notes:"Heaviest pack of the plan, 10–15% incline. This is the pack-out."}}
};
const BLOCK_GYM_BLURBS={1:"Own the big barbell lifts: back squat, trap-bar deadlift, bench, cable row, farmer handles, incline treadmill with a pack.",2:"Change the angle: front squat, barbell RDL, incline dumbbell bench, chest-supported row, trap-bar carry, stair climber with a pack.",3:"Peak: Bulgarian split squat, single-leg RDL, overhead press, lat pulldown or weighted pull-ups, sandbag carry, heaviest pack on the steepest incline."};
const BLOCKS={
  1:{name:"Foundation",blurb:"Own the basic patterns: goblet squat, RDL, floor press, one-arm row, farmer carry."},
  2:{name:"Unilateral",blurb:"One leg and one arm at a time: split squat, staggered RDL, push-ups, gorilla row, suitcase carry, hillier rucks."},
  3:{name:"Peak",blurb:"Hardest variants and heaviest pack: Bulgarian split squat, single-leg RDL, single-arm floor press, renegade row, rack carry."}
};
const BLOCK_VARIANTS={
  "Goblet Squat":{2:{name:"Split Squat",min:8,max:10,suffix:"/leg"},3:{name:"Bulgarian Split Squat",min:6,max:8,suffix:"/leg"}},
  "DB Romanian Deadlift":{2:{name:"Staggered-Stance RDL",min:8,max:10,suffix:"/side"},3:{name:"Single-Leg RDL",min:6,max:8,suffix:"/leg"}},
  "DB Floor Press":{2:{name:"Push-Ups",min:10,max:15,bodyweightOnly:true},3:{name:"Single-Arm Floor Press",min:8,max:10,suffix:"/side"}},
  "1-Arm DB Row":{2:{name:"Gorilla Row",min:8,max:10,suffix:"/side"},3:{name:"Renegade Row",min:6,max:8,suffix:"/side"}},
  "Farmer Carry":{2:{name:"Suitcase Carry",prescription:"4 rounds × 40 sec/side",sets:8},3:{name:"Rack Carry",prescription:"5 × 40 sec",sets:5}},
  "Weighted Walk / Ruck":{2:{minutes:35,prescription:"35 min",notes:"Same load as last block, add hills. Brisk pace, no running."},3:{minutes:40,prescription:"40 min",notes:"Heaviest pack of the plan (work toward 25–30 lb). Keep posture tall; this is the hunt."}}
};
function currentBlock(){ const p=huntPlan(); if(p) return p.phase==='base'?1:p.phase==='build'?2:3; const start=new Date((localStorage.hr30_startDate||localDateKey())+'T00:00:00'); const wk=Math.floor(Math.max(0,new Date()-start)/604800000); return (Math.floor(wk/4)%3)+1; }
function blockInfo(){ const b=currentBlock(), p=huntPlan(); let until=''; if(p){ if(p.phase==='base') until=`switches to Block 2 in ${Math.max(0,p.days-59)} days (Build phase)`; else if(p.phase==='build') until=`switches to Block 3 in ${Math.max(0,p.days-24)} days (Peak phase)`; else until='final block through the hunt'; } else { const start=new Date((localStorage.hr30_startDate||localDateKey())+'T00:00:00'); const days=Math.floor(Math.max(0,new Date()-start)/86400000); until=`switches in ${28-(days%28)} days`; } return {n:b, ...BLOCKS[b], until}; }
function applyBlock(e,block,mode){ if(mode==='bodyweight') return e; const v=(mode==='gym'?GYM_BLOCK_VARIANTS:BLOCK_VARIANTS)[e.name]?.[block]; if(!v) return e; const x={...e,...v,blockOf:e.name,block}; if(x.type==='strength'&&v.min){ x.prescription=`${x.sets} × ${x.min}${x.max>x.min?'–'+x.max:''}${v.suffix||''}`; } return x; }
function blockVariantsFor(e,mode='dumbbells'){ return [2,3].map(b=>applyBlock(e,b,mode)).filter(x=>x!==e); }
...
function effectiveWorkout(day){
  ...
  w.mode=mode; const blk=currentBlock(); w.block=blk; w.exercises=w.exercises.map(e=>applyBlock(substituteExercise(e,mode),blk,mode));
  let trimmed=false;
...
function countdownCard(){
  ...
  return `<section class="card countdown prime phase-${p.phase}"><div class="cd-top"><div class="kicker"><span class="led"></span>Next hunt · ${esc(p.hunt.name||'Opening day')} <span class="tag" style="margin-left:6px">${esc(WEAPONS[p.hunt.weapon]||'Bow')}</span></div><div class="phase-badge">${p.label}</div></div>
...
const WEAPONS={bow:"Bow",rifle:"Rifle",crossbow:"Crossbow",muzzleloader:"Muzzleloader"};
let editingHuntId=null;
function openHunts(editId=null){
  editingHuntId=editId||null; const hunts=getHunts(), today=localDateKey(), h=editingHuntId?hunts.find(x=>x.id===editingHuntId)||{}:{};
  openModal(`<div class="close-row"><div><div class="kicker">HUNTS</div><h2>${editingHuntId?'Edit hunt':'Opening days'}</h2><p class="note">The next upcoming hunt drives the plan.</p></div><button class="icon-btn" onclick="closeModal()">×</button></div>
    <div class="field"><label>Hunt name</label><input id="huntName" value="${esc(h.name||'')}" placeholder="e.g. Rifle opener, Late-season bow"></div>
    <div class="set-grid"><div class="field"><label>Opening day</label><input id="huntDate" type="date" value="${h.date||''}" ${editingHuntId?'':`min="${today}"`}></div><div class="field"><label>Weapon</label><select id="huntWeapon">${Object.entries(WEAPONS).map(([k,v])=>`<option value="${k}" ${(h.weapon||'bow')===k?'selected':''}>${v}</option>`).join('')}</select></div></div>
    <div class="field"><label>Notes (optional)</label><textarea id="huntNotes" rows="2" placeholder="Unit, property, who's going">${esc(h.notes||'')}</textarea></div>
    ${editingHuntId?`<div class="action-row"><button class="secondary" onclick="openHunts()">CANCEL</button><button class="primary" style="margin-top:0" onclick="saveHunt()">SAVE CHANGES</button></div>`:`<button class="primary" onclick="saveHunt()">ADD HUNT</button>`}
    <div class="divider"></div><div class="kicker">YOUR HUNTS</div><div class="field-list">${hunts.length?hunts.map(x=>{const past=x.date<today, d=daysUntil(x.date);return `<div class="archery-item ${past?'past':''}"><div class="top"><strong>${esc(x.name||'Hunt')}</strong><small>${formatDate(new Date(x.date+'T00:00:00'))}</small></div><p class="note"><span class="tag">${esc(WEAPONS[x.weapon]||'Bow')}</span> ${past?'Done':d===0?'Today':`${d} days out`}${x.notes?` • ${esc(x.notes)}`:''}</p><div style="display:flex;gap:16px"><button class="ghost" onclick="openHunts('${x.id}')">Edit</button><button class="ghost" onclick="deleteHunt('${x.id}')">Remove</button></div></div>`;}).join(''):'<div class="empty">No hunts yet.</div>'}</div>`);
}
function saveHunt(){ const name=$('#huntName').value.trim(), date=$('#huntDate').value, weapon=$('#huntWeapon').value, notes=$('#huntNotes').value.trim(); if(!date){alert('Pick an opening day.');return;} const hunts=STORE.get('hr30_hunts',[]); if(editingHuntId){ const x=hunts.find(h=>h.id===editingHuntId); if(x){ Object.assign(x,{name:name||'Hunt',date,weapon,notes,updatedAt:new Date().toISOString()}); } } else hunts.push({id:uid(),name:name||'Hunt',date,weapon,notes,createdAt:new Date().toISOString()}); STORE.set('hr30_hunts',hunts); if(weapon==='rifle'||weapon==='muzzleloader') ensureRifleChecklist(); editingHuntId=null; openHunts(); }
function deleteHunt(id){ if(!confirm('Remove this hunt?')) return; STORE.set('hr30_hunts',getHunts().filter(h=>h.id!==id)); openHunts(); }
function ensureRifleChecklist(){ const a=getState().checklist; if(a.some(i=>i.category==='Rifle')) return; ["Rifle zeroed at your hunting distance","Ammo: same load you zeroed with","Scope covers / lens cloth","Sling","Cleaning kit + bore snake","Ear protection","Rifle case","Shooting sticks / bipod"].forEach(label=>a.push({id:uid(),category:'Rifle',label,done:false})); STORE.set('hr30_checklist',a); }
const SCENES={
...
if(!localStorage.hr30_onboarded){ setTimeout(()=>openOnboarding(0),150); }
...
function notesMarkup(notes,cls='steps'){ if(!notes) return ''; const parts=String(notes).split(/\s+•\s+/); if(parts.length<3) return `<p class="note">${esc(notes)}</p>`; const last=parts[parts.length-1]; const tail=/^(do not|don.t|keep|stay|no |rest)/i.test(last)?parts.pop():''; return `<ul class="${cls}">${parts.map(x=>`<li>${esc(x.replace(/\.$/,''))}</li>`).join('')}</ul>${tail?`<p class="note">${esc(tail)}</p>`:''}`; }
function exerciseList(workout){ return workout.exercises.map((e,i)=>`<div class="exercise"><div class="exercise-num">${i+1}</div><div style="flex:1"><h4>${esc(e.name)}</h4><p>${esc(e.prescription)}</p><div class="tag-row">${e.rest?`<span class="tag">${e.rest}s rest</span>`:''}</div>${notesMarkup(e.notes)}${exerciseActionButtons(e.name)}</div></div>`).join(''); }
...
function renderToday(){
  ...
  $('#main').innerHTML=`
    ...
    <section class="card scripture-card on-scene"><div class="row" style="display:flex;justify-content:space-between;align-items:baseline"><div class="scripture-ref">${verse.ref}</div><span class="tag">${TRANSLATIONS[bibleVersion()]}</span></div><p class="scripture-text">“${esc(verseText(verse)).replace(/LORD/g,'<span class="lord">Lord</span>')}”</p><div class="scripture-focus">TRAIN UNDER THIS: ${verse.focus}</div></section>
    ${readinessCard(ready)}
    ${countdownCard()}
    ${newBlockNotice()}
    ${(()=>{ const done=todaysSessions().find(x=>x.duration>0)||todaysSessions()[0]; if(done) return completedCard(done,w,d); return `<section class="card"><div class="kicker">${planLabel()} • ${d}${w.taper?' • TAPER':''}${w.readinessAdj?' • ADJUSTED':''}</div><h2 style="margin:6px 0">${w.title}</h2><p class="sub">${w.focus}</p>${readinessBanner(w)}${w.duration?`<div class="mode-row"><span class="tag mode-tag">${modeLabel(w.mode)}${hasTodayOverride()?' • today only':''}</span><div style="display:flex;gap:14px;flex-wrap:wrap;justify-content:flex-end">${hasTodayOverride()?`<button class="ghost" onclick="setTodayOverride()">Back to ${modeLabel(baseEquipment()).toLowerCase()} →</button>`:['dumbbells','bodyweight','gym'].filter(m=>m!==w.mode).map(m=>`<button class="ghost" onclick="setTodayOverride('${m}')">${m==='bodyweight'?'Bodyweight':m==='gym'?'Gym':'Dumbbells'} today →</button>`).join('')}</div></div>`:''}${w.duration?`<button class="primary" onclick="startWorkout('${d}')">START WORKOUT • ${w.duration} MIN</button>`:`<button class="primary" onclick="quickLogRecovery()">LOG RECOVERY DAY</button>`}<div style="margin-top:10px">${exerciseList(w)}</div></section>`; })()}
    <div class="section-title"><h3>Field tools</h3><small>build the whole hunter</small></div>
    <div class="quick-grid"><button class="quick-card" onclick="openArcheryLog()"><span class="quick-icon">${ICONS.target}</span><strong>Archery Lab</strong><small>Log, target photos, trends, form check.</small></button><button class="quick-card" onclick="openWeaponProfile()"><span class="quick-icon">${ICONS.weapons}</span><strong>Weapon Profile</strong><small>Bow and rifle setups in one place.</small></button><button class="quick-card" onclick="openHuntPrep()"><span class="quick-icon">${ICONS.check}</span><strong>Hunt Prep</strong><small>Gear, legal, field, clothing.</small></button><button class="quick-card" onclick="openHunts()"><span class="quick-icon">${ICONS.hourglass}</span><strong>Hunt Dates</strong><small>Opening days drive the plan.</small></button><button class="quick-card wide" onclick="tab('fuel')"><span class="quick-icon">${ICONS.fuel}</span><strong>Fuel</strong><small>Meals, macros, photo log.</small></button</div><p class="note" style="text-align:center;margin:18px 0 4px">Beta · <a href="${feedbackHref()}" style="color:var(--accent-2)">Send feedback</a> · <a href="#" onclick="openStartGuide();return false" style="color:var(--accent-2)">Start guide</a>`;
}
const ICONS={
  target:`<svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="22" cy="26" r="16"/><circle cx="22" cy="26" r="10"/><circle cx="22" cy="26" r="4" fill="currentColor" stroke="none"/><path d="M22 26 L42 6"/><path d="M34 6 L42 6 L42 14"/></svg>`,
  weapons:`<svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M10 8 C30 12 36 20 40 40"/><path d="M10 8 L40 40"/><path d="M6 40 L26 20"/><path d="M26 20 L34 12 L38 10 L36 14 L28 22 Z" fill="currentColor"/><path d="M6 40 L10 36 L14 40 L10 44 Z" fill="currentColor"/></svg>`,
  check:`<svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><rect x="8" y="6" width="32" height="36" rx="4"/><path d="M16 6 V4 H32 V6"/><path d="M15 22 L21 28 L33 16"/><path d="M15 34 H33"/></svg>`,
  hourglass:`<svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 6 H36"/><path d="M12 42 H36"/><path d="M15 6 C15 18 24 20 24 24 C24 28 15 30 15 42"/><path d="M33 6 C33 18 24 20 24 24 C24 28 33 30 33 42"/><path d="M19 12 L29 12 L24 20 Z" fill="currentColor" stroke="none"/><path d="M17 40 L31 40 L24 30 Z" fill="currentColor" stroke="none"/><path d="M24 24 V30" stroke-dasharray="1.5 2.5"/></svg>`,
  fuel:`<svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M24 6 C30 14 36 18 36 27 A12 12 0 0 1 12 27 C12 20 17 16 18 12 C20 16 23 17 24 6 Z"/><path d="M24 40 A6 6 0 0 0 30 33 C30 29 27 27 24 22 C21 27 18 29 18 33 A6 6 0 0 0 24 40 Z" fill="currentColor" stroke="none"/></svg>`,
  bow:`<svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 6 C30 14 30 34 12 42"/><path d="M12 6 L12 42"/><path d="M12 24 H40"/><path d="M40 24 L34 20 M40 24 L34 28"/></svg>`,
  rifle:`<svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 30 L30 30 L44 22 L44 18 L28 18 L26 22 L6 22 Z"/><path d="M6 30 L4 40 L10 40 L14 30"/><path d="M18 18 V14 H26 V18"/><circle cx="22" cy="12" r="3"/></svg>`
};
const COMMENDATIONS=[
  {ref:"2 Timothy 4:7",kjv:"I have fought a good fight, I have finished my course, I have kept the faith.",bsb:"I have fought the good fight, I have finished the race, I have kept the faith.",web:"I have fought the good fight. I have finished the course. I have kept the faith."},
  {ref:"Galatians 6:9",kjv:"And let us not be weary in well doing: for in due season we shall reap, if we faint not.",bsb:"Let us not grow weary in well-doing, for in due time we will reap a harvest if we do not give up.",web:"Let us not be weary in doing good, for we will reap in due season, if we don’t give up."},
  {ref:"Proverbs 21:31",kjv:"The horse is prepared against the day of battle: but safety is of the LORD.",bsb:"A horse is prepared for the day of battle, but victory is of the LORD.",web:"The horse is prepared for the day of battle; but victory is with Yahweh."},
  {ref:"Matthew 25:21",kjv:"Well done, thou good and faithful servant: thou hast been faithful over a few things, I will make thee ruler over many things.",bsb:"Well done, good and faithful servant! You have been faithful with a few things; I will put you in charge of many things.",web:"Well done, good and faithful servant. You have been faithful over a few things, I will set you over many things."},
  {ref:"1 Corinthians 15:58",kjv:"Be ye stedfast, unmoveable, always abounding in the work of the Lord, forasmuch as ye know that your labour is not in vain in the Lord.",bsb:"Be steadfast and immovable. Always excel in the work of the Lord, because you know that your labor in the Lord is not in vain.",web:"Be steadfast, immovable, always abounding in the Lord’s work, because you know that your labor is not in vain in the Lord."},
  {ref:"Proverbs 24:5",kjv:"A wise man is strong; yea, a man of knowledge increaseth strength.",bsb:"A wise man is strong, and a man of knowledge enhances his strength.",web:"A wise man has great power. A knowledgeable man increases strength."}
];
function todaysCommendation(){ const d=new Date(); const day=Math.floor((d-new Date(d.getFullYear(),0,0))/86400000); return COMMENDATIONS[day%COMMENDATIONS.length]; }
function todaysSessions(){ const today=localDateKey(); return getState().history.filter(h=>localDateKey(new Date(h.date))===today); }
function completedCard(sess,w,d){ const v=todaysCommendation(), blocks=(sess.logs||[]).length, moved=(sess.logs||[]).filter(l=>l.weight).reduce((a,l)=>a+(l.weight||0)*(l.reps||1),0); return `<section class="card done-card"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px"><div><div class="kicker"><span class="led"></span>Done · ${planLabel()} · ${d}</div><h2 style="margin:6px 0">${esc(sess.title)}</h2></div><span class="phase-badge">Complete</span></div><div class="stat-strip" style="margin-top:12px"><div class="metric"><strong>${sess.duration}</strong><span>Minutes</span></div><div class="metric"><strong>${blocks}</strong><span>Sets · blocks</span></div><div class="metric"><strong>${moved?Math.round(moved).toLocaleString():'—'}</strong><span>${moved?'Lb moved':'Bodyweight'}</span></div></div><div class="commend"><div class="scripture-ref">${v.ref}</div><p class="scripture-text" style="font-size:16px">“${esc(verseText(v)).replace(/LORD/g,'<span class="lord">Lord</span>')}”</p><p class="note">Well done. The work is logged and the day is stewarded. Rest, eat, and be present for the people who need you.</p></div>${sess.summary?`<div class="notice" style="margin-top:12px">${esc(sess.summary)}</div>`:''}<div style="display:flex;gap:16px;margin-top:12px;flex-wrap:wrap"><button class="ghost" onclick="viewHistory('${sess.id||''}','${sess.date}')">View details →</button><button class="ghost" onclick="openSessionEditor('${sess.id||''}')">Edit</button><button class="ghost" onclick="startWorkout('${d}')">Train again</button></div></section>`; }
function newBlockNotice(){ if(baseEquipment()==='bodyweight') return ''; const b=blockInfo(); const seen=Number(localStorage.hr30_lastBlock||0); if(!seen){ localStorage.hr30_lastBlock=b.n; return ''; } if(seen===b.n) return ''; return `<section class="card prime"><div class="kicker"><span class="led"></span>New training block</div><h3 style="margin:6px 0 4px">Block ${b.n} · ${b.name}</h3><p class="sub">${baseEquipment()==='gym'?BLOCK_GYM_BLURBS[b.n]:b.blurb}</p><p class="note" style="margin-top:8px">Open Learn for the drawings and videos before your first session. Loads start fresh for new movements.</p><button class="secondary" style="width:100%;margin-top:10px" onclick="localStorage.hr30_lastBlock=${b.n};renderToday()">GOT IT</button></section>`; }
// ---- First launch, start guide, feedback ----
const DEFAULT_AI_ENDPOINT='https://hunt-ready-30.vercel.app/api/analyze-meal';
const FEEDBACK_EMAIL='fabiano@menofrefuge.com';
function isInstalled(){ return !!(window.navigator.standalone || (window.matchMedia&&window.matchMedia('(display-mode: standalone)').matches)); }
function isIOS(){ return /iPhone|iPad|iPod/.test(navigator.userAgent); }
let onboardStep=0;
const ONBOARD_STEPS=['welcome','hunt','equipment','guide','done'];
function openOnboarding(step=0){ onboardStep=step; openModal(onboardMarkup()); const m=$('#modal'); if(m) m.scrollTop=0; }
function onboardMarkup(){
  const key=ONBOARD_STEPS[onboardStep], n=onboardStep+1, total=ONBOARD_STEPS.length, s=getState();
  const dots=`<div class="ticks" style="margin:0 0 14px">${ONBOARD_STEPS.map((_,i)=>`<i class="${i<=onboardStep?'on':''}"></i>`).join('')}</div>`;
  const nav=(nextLabel='CONTINUE',skip=false)=>`<div class="action-row" style="margin-top:16px">${onboardStep>0?`<button class="secondary" onclick="onboardPrev()">BACK</button>`:`<span></span>`}<button class="primary" style="margin-top:0" onclick="onboardNext()">${nextLabel}</button></div>${skip?`<button class="ghost" style="width:100%;margin-top:10px;text-align:center" onclick="onboardNext(true)">Skip for now</button>`:''}`;
  let body='';
  if(key==='welcome') body=`<div class="kicker">Step ${n} of ${total}</div><h2 style="margin:6px 0 10px">Get hunt ready in 30 minutes a day</h2>${dots}<p class="sub">HUNT READY 30 is a faith-rooted training plan for hunters. Short strength sessions with dumbbells or your own bodyweight, rucks that build pack-out legs, a daily verse, and archery tools that use AI to read your target and your form.</p><p class="sub" style="margin-top:10px">It takes two minutes to set up. Everything you enter stays on this phone.</p><div class="notice" style="margin-top:12px"><strong>Beta build.</strong> You're one of a handful of men testing this before it goes public. There's a Send Feedback button in Settings; use it freely.</div>${nav('LET\'S GO')}`;
  if(key==='hunt'){ const d=new Date(); d.setDate(d.getDate()+84); body=`<div class="kicker">Step ${n} of ${total}</div><h2 style="margin:6px 0 10px">Your first hunt</h2>${dots}<p class="sub">The whole plan counts back from opening day: Base, Build, Peak, then a taper so you arrive fresh. Pick the first hunt you're training for. You can add more later.</p><div class="field"><label>Hunt name</label><input id="obHuntName" placeholder="e.g. Rifle opener, Late-season bow"></div><div class="set-grid"><div class="field"><label>Opening day</label><input id="obHuntDate" type="date" min="${localDateKey()}" value="${localDateKey(d)}"></div><div class="field"><label>Weapon</label><select id="obWeapon">${Object.entries(WEAPONS).map(([k,v])=>`<option value="${k}">${v}</option>`).join('')}</select></div></div>${nav('SAVE HUNT',true)}`; }
  if(key==='equipment') body=`<div class="kicker">Step ${n} of ${total}</div><h2 style="margin:6px 0 10px">What do you train with?</h2>${dots}<p class="sub">Pick what you have at home most days. You can switch any day from the Today screen, so a hotel room or deer camp doesn't break the plan.</p><div class="field"><label>Equipment</label><select id="obEquip"><option value="dumbbells" ${baseEquipment()==='dumbbells'?'selected':''}>Dumbbells + a weight vest or backpack</option><option value="bodyweight" ${baseEquipment()==='bodyweight'?'selected':''}>Bodyweight + a loaded backpack</option><option value="gym" ${baseEquipment()==='gym'?'selected':''}>Gym membership (barbells, cables, machines)</option></select></div><label class="check-row" style="border:0"><input type="checkbox" id="obBands" ${s.settings.hasBands?'checked':''}><span>I also have resistance bands</span></label><div class="set-grid"><div class="field"><label>Body weight (lb, optional)</label><input id="obWeight" inputmode="decimal" placeholder="e.g. 210"></div><div class="field"><label>Starting ruck load (lb)</label><input id="obRuck" inputmode="numeric" value="${s.preferredRuck||10}"></div></div>${nav()}`;
  if(key==='guide') body=`<div class="kicker">Step ${n} of ${total}</div><h2 style="margin:6px 0 10px">How to use it</h2>${dots}${startGuideMarkup()}${nav('FINISH')}`;
  if(key==='done') body=`<div class="kicker">Ready</div><h2 style="margin:6px 0 10px">You're set up.</h2><p class="sub">Today's verse, readiness check, countdown, and workout are waiting on the Today screen. Train hard, stay honest, and send feedback when something feels off.</p><div class="commend" style="margin-top:14px"><div class="scripture-ref">Proverbs 21:31</div><p class="scripture-text" style="font-size:16px">“${esc(verseText(COMMENDATIONS[2])).replace(/LORD/g,'<span class="lord">Lord</span>')}”</p></div><button class="primary" onclick="finishOnboarding()">OPEN TODAY</button>`;
  return `<div class="onboard">${body}</div>`;
}
function startGuideMarkup(){ const installed=isInstalled(), ios=isIOS();
  return `<ol class="guide-steps">
    <li><strong>${installed?'Installed on your Home Screen. ✓':'Put it on your Home Screen.'}</strong>${installed?'':` ${ios?'In Safari tap the Share button, then <em>Add to Home Screen</em>.':'In your browser menu choose <em>Add to Home Screen</em> or <em>Install app</em>.'} It runs full-screen and works offline.`}</li>
...
function onboardPrev(){ if(onboardStep>0) openOnboarding(onboardStep-1); }
function onboardNext(skip=false){
  const key=ONBOARD_STEPS[onboardStep];
  if(key==='hunt'&&!skip){ const date=$('#obHuntDate').value; if(!date){ alert('Pick an opening day, or skip for now.'); return; } const hunts=STORE.get('hr30_hunts',[]); hunts.push({id:uid(),name:$('#obHuntName').value.trim()||'First hunt',date,weapon:$('#obWeapon').value,notes:'',createdAt:new Date().toISOString()}); STORE.set('hr30_hunts',hunts); if(['rifle','muzzleloader'].includes($('#obWeapon').value)) ensureRifleChecklist(); }
  if(key==='equipment'){ const st=getState().settings; st.equipment=$('#obEquip').value; st.hasBands=$('#obBands').checked; if(!st.aiEndpoint) st.aiEndpoint=DEFAULT_AI_ENDPOINT; STORE.set('hr30_settings',st); localStorage.hr30_preferredRuck=+$('#obRuck').value||10; const w=+$('#obWeight').value||0; if(w){ const h=getState().bodyHistory; h.unshift({date:new Date().toISOString(),weight:w,waist:0}); STORE.set('hr30_bodyHistory',h.slice(0,365)); } }
  openOnboarding(Math.min(ONBOARD_STEPS.length-1,onboardStep+1));
}
function finishOnboarding(){ localStorage.hr30_onboarded=APP_VERSION; localStorage.hr30_lastBlock=currentBlock(); closeModal(); tab('today',false); }
function openStartGuide(){ openModal(`<div class="close-row"><div><div class="kicker">START GUIDE</div><h2>How to use HUNT READY 30</h2></div><button class="icon-btn" onclick="closeModal()">×</button></div>${startGuideMarkup()}<button class="secondary" style="width:100%;margin-top:14px" onclick="openOnboarding(0)">RUN SETUP AGAIN</button>`); }
function feedbackHref(){ const s=getState(), p=huntPlan(); const body=[`HUNT READY 30 feedback`,``,`What happened / what I expected:`,``,``,`--- details (leave this in) ---`,`Version: ${APP_VERSION}`,`Installed: ${isInstalled()?'Home Screen':'browser'}`,`Device: ${navigator.userAgent}`,`Equipment: ${baseEquipment()}${s.settings.hasBands?' + bands':''}`,`Plan: ${planLabel()}${p?` · ${p.days} days to ${p.hunt.name||'hunt'}`:''}`,`Sessions logged: ${s.history.length}`].join('\n'); return `mailto:${FEEDBACK_EMAIL}?subject=${encodeURIComponent('HUNT READY 30 feedback · v'+APP_VERSION)}&body=${encodeURIComponent(body)}`; }
const READINESS_WHY="Your pre-flight check. Rate how you actually feel this morning and the app matches the workout to it: push on a good day, trim on a rough one. Some days you find out what you're made of. No day is worth getting hurt over. This is about longevity, not ego: hunt ready, and still standing for your family.";
...
function readinessCard(ready){ return `<section class="card ${ready?'':'prime'}"><div class="section-title" style="margin:0 0 6px"><h3>Readiness check</h3><small>${ready?`${ready.score} today`:'First thing · 30 seconds'}</small></div>${ready?readinessSummary(ready):`<p class="note" style="margin:0 0 12px">${READINESS_WHY}</p>`+readinessForm()}</section>`; }
...
function readinessSummary(r){ const lvl=readinessLevel(r.score); const verdict={ready:'Ready to work',honest:'Train, but stay honest',conservative:'Trimmed day',recovery:'Recovery recommended'}[lvl]; const effect={ready:'Train as written. Push where the form stays clean.',honest:'Do the session as written, but skip load jumps today.',conservative:'One fewer set per lift, hold your loads, shorter carries. Already applied to today\'s workout.',recovery:'The app suggests the easy day instead. Training hard on a day like this is where injuries happen.'}[lvl]; return `<div class="score-wrap"><div class="score-ring" style="--score:${r.score}"><strong>${r.score}</strong></div><div><strong>${verdict}</strong><p class="note">${effect}</p><p class="note mono" style="margin-top:4px">Sleep ${r.sleep} · Energy ${r.energy} · Soreness ${r.soreness} · Stress ${r.stress}</p><div style="display:flex;gap:16px;flex-wrap:wrap"><button class="ghost" onclick="editReadiness()">Edit check-in →</button><button class="ghost" onclick="const w=$('#readinessWhy'); w.hidden=!w.hidden">Why this matters</button></div><p id="readinessWhy" class="note" hidden style="margin-top:8px">${READINESS_WHY}</p></div></div>`; }
...
function blockCard(){ const b=blockInfo(); if(baseEquipment()==='bodyweight') return `<section class="card" style="margin-bottom:14px"><div class="kicker">Progression</div><h3 style="margin:6px 0 4px">Ladders, not blocks</h3><p class="sub">In bodyweight mode each main lift climbs its own five-step ladder, so the movement changes as you earn it.</p></section>`; return `<section class="card prime" style="margin-bottom:14px"><div class="row" style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px"><div><div class="kicker"><span class="led"></span>Training block</div><h3 style="margin:6px 0 4px">Block ${b.n} · ${b.name}</h3></div><span class="phase-badge">${b.n} / 3</span></div><p class="sub">${baseEquipment()==='gym'?BLOCK_GYM_BLURBS[b.n]:b.blurb}</p><p class="note" style="margin-top:8px">Every four weeks the movement in each slot changes so you keep adapting. This block ${b.until}.</p><div class="ticks" style="margin-top:10px">${[1,2,3].map(i=>`<i class="${i<=b.n?'on':''}"></i>`).join('')}</div></section>`; }
function renderWeekMarkup(){ const days=['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'],today=dayName(); return `${blockCard()}<div class="day-grid">${days.map(d=>`<div class="day-card ${d===today?'today':''}"><div class="kicker">${d}</div><h4>${PROGRAM[d].title}</h4><p>${PROGRAM[d].focus}</p><button class="ghost" onclick="openDay('${d}')">View →</button></div>`).join('')}</div>${huntPlan()?phaseTimeline(huntPlan()):`<section class="card" style="margin-top:14px"><div class="kicker">PROGRESSION</div><h3>Weeks 1–2</h3><p class="sub">Learn the movements. Finish with 2–3 clean reps in reserve.</p><h3>Weeks 3–4</h3><p class="sub">Add reps or 2.5–5 lb when form stays clean.</p><h3>Weeks 5–8</h3><p class="sub">Push strength and bring ruck load gradually toward 20–30 lb.</p></section>`}`; }
...
function openPastSession(){ openSessionEditor(null); }
function sessionExercises(day,mode){ const blk=currentBlock(); return PROGRAM[day].exercises.map(e=>applyBlock(substituteExercise(e,mode),blk,mode)).filter(e=>e.type!=='rest'); }
function sessionSetsMarkup(day,mode,logs){
  const groups={}; (logs||[]).forEach(l=>(groups[l.name]??=[]).push(l));
  return sessionExercises(day,mode).map((e,i)=>{ const g=groups[e.name]||[], first=g[0]||{}, n=g.length||'', id=`se${i}`;
    const num=(lab,key,val,ph='')=>`<div class="field"><label>${lab}</label><input id="${id}-${key}" inputmode="decimal" value="${val??''}" placeholder="${ph}"></div>`;
    let inputs='';
    if(e.type==='strength'&&e.ladder) inputs=`<div class="field"><label>Level</label><select id="${id}-level">${e.ladder.map((nm,k)=>`<option value="${k+1}" ${(first.level||1)===k+1?'selected':''}>L${k+1} · ${esc(nm)}</option>`).join('')}</select></div><div class="set-grid">${num('Reps','reps',first.reps,e.min)}${num('Sets','sets',n,e.sets)}</div>`;
    else if(e.type==='strength'&&e.band) inputs=`<div class="field"><label>Band</label><select id="${id}-band">${BAND_LEVELS.map(bn=>`<option ${(first.band||'Medium')===bn?'selected':''}>${bn}</option>`).join('')}</select></div><div class="set-grid">${num('Reps','reps',first.reps,e.min)}${num('Sets','sets',n,e.sets)}</div>`;
    else if(e.type==='strength') inputs=`<div class="set-grid three">${num(e.barbell?'Bar wt (lb)':'Weight (lb)','weight',first.weight,e.barbell?'incl. bar':'')}${num('Reps','reps',first.reps,e.min)}${num('Sets','sets',n,e.sets)}</div>`;
    else if(e.type==='carry') inputs=`<div class="set-grid three">${num('Load (lb)','weight',first.weight)}${num('Seconds','seconds',first.seconds,e.seconds)}${num('Carries','sets',n,e.sets)}</div>`;
    else if(e.type==='timed') inputs=`<div class="set-grid">${num('Seconds','seconds',first.seconds,e.seconds)}${num('Sets','sets',n,e.sets)}</div>`;
    else if(e.type==='ruck') inputs=`<div class="set-grid three">${num('Load (lb)','weight',first.weight)}${num('Minutes','minutes',first.minutes,e.minutes)}${num('Miles','distance',first.distance)}</div>`;
    else if(e.type==='circuit') inputs=`<div class="set-grid">${num('Rounds','rounds',first.rounds)}${num('Minutes','minutes',first.minutes,e.minutes)}</div>${circuitLoadInputs(e,id+'-load',first.loads||{})}`;
    else inputs=`<label class="check-row" style="border:0;padding:6px 0"><input type="checkbox" id="${id}-done" ${g.length?'checked':''}><span>Done</span></label>`;
    return `<div class="session-ex" data-name="${esc(e.name)}" data-type="${e.type}" data-idx="${i}"><div class="kicker">${i+1} · ${esc(e.name)} <span class="mono" style="letter-spacing:0;color:var(--muted)">${esc(e.prescription)}</span></div>${inputs}</div>`; }).join('');
}
function openSessionEditor(id){
  editingSessionId=id||null; const x=id?getState().history.find(h=>h.id===id):null; const when=x?new Date(x.date):new Date(); const today=localDateKey();
  const day=x?x.day:dayName(), mode=x?(x.mode||'dumbbells'):equipmentMode();
  const hh=String(when.getHours()).padStart(2,'0'), mm=String(x?when.getMinutes():Math.max(0,when.getMinutes()-30)).padStart(2,'0');
  openModal(`<div class="close-row"><div><div class="kicker">HISTORY</div><h2>${x?'Edit session':'Log a past session'}</h2><p class="note">${x?'Change the date, time, or what you did. Weights you enter update your current loads on Progress.':'For a workout you did without the app, or to restore one that was lost. Weights you enter update your current loads on Progress.'}</p></div><button class="icon-btn" onclick="closeModal()">×</button></div>
  <div class="set-grid"><div class="field"><label>Date</label><input id="psDate" type="date" value="${localDateKey(when)}" max="${today}"></div><div class="field"><label>Time</label><input id="psTime" type="time" value="${hh}:${mm}"></div></div>
  <div class="set-grid"><div class="field"><label>Workout</label><select id="psDay" onchange="rerenderSessionSets()">${Object.keys(PROGRAM).map(d=>`<option value="${d}" ${d===day?'selected':''}>${d} · ${PROGRAM[d].title}</option>`).join('')}</select></div><div class="field"><label>Duration (min)</label><input id="psDuration" inputmode="numeric" value="${x?x.duration:30}"></div></div>
  <div class="field"><label>Equipment</label><select id="psMode" onchange="rerenderSessionSets()"><option value="dumbbells" ${mode==='dumbbells'?'selected':''}>Dumbbells</option><option value="bodyweight" ${mode==='bodyweight'?'selected':''}>Bodyweight</option><option value="gym" ${mode==='gym'?'selected':''}>Gym</option></select></div>
  <div class="divider"></div><div class="kicker">SETS &amp; BLOCKS</div><p class="note" style="margin:6px 0 4px">Fill in what you did. Leave an exercise blank to skip it.</p><div id="sessionSets">${sessionSetsMarkup(day,mode,x?.logs)}</div>
  <div class="field"><label>Notes (optional)</label><textarea id="psNotes" rows="2" placeholder="e.g. felt hard but grateful">${esc(x?String(x.summary||'').replace(/^Logged afterward:?\s*/,''):'')}</textarea></div>
  <button class="primary" onclick="saveSessionEditor()">${x?'SAVE CHANGES':'LOG SESSION'}</button>${x?`<button class="secondary danger" style="width:100%;margin-top:10px" onclick="deleteSession('${x.id}')">DELETE SESSION</button>`:''}`);
}
function rerenderSessionSets(){ const el=$('#sessionSets'); if(el) el.innerHTML=sessionSetsMarkup($('#psDay').value,$('#psMode').value,null); }
function collectSessionLogs(){ const logs=[]; document.querySelectorAll('#sessionSets .session-ex').forEach(box=>{ const name=box.dataset.name,type=box.dataset.type,id='se'+box.dataset.idx; const v=k=>{ const el=$(`#${id}-${k}`); return el?el.value:''; }; const n=k=>Number(v(k))||0;
  if(type==='strength'){ const level=$(`#${id}-level`)?Number(v('level')):0, band=$(`#${id}-band`)?v('band'):'', weight=n('weight'), reps=n('reps'), sets=n('sets'); if(!(weight||reps||level||band)||!sets) return; const def=exerciseDef(name,$('#psMode').value); for(let i=1;i<=sets;i++){ const l={name,set:i,type}; if(level){l.level=level;l.levelName=def?.ladder?def.ladder[level-1]:'';} else if(band) l.band=band; else l.weight=weight; l.reps=reps; logs.push(l);} }
  else if(type==='carry'){ const sets=n('sets'); if(!sets&&!n('weight')) return; for(let i=1;i<=Math.max(1,sets);i++) logs.push({name,set:i,type,weight:n('weight'),seconds:n('seconds')}); }
  else if(type==='timed'){ const sets=n('sets'); if(!sets&&!n('seconds')) return; for(let i=1;i<=Math.max(1,sets);i++) logs.push({name,set:i,type,seconds:n('seconds')}); }
  else if(type==='ruck'){ if(!(n('weight')||n('minutes')||n('distance'))) return; logs.push({name,set:1,type,weight:n('weight'),minutes:n('minutes'),distance:n('distance')}); }
  else if(type==='circuit'){ const loads=readCircuitLoads(id+'-load'); if(!(n('rounds')||n('minutes')||Object.keys(loads).length)) return; const l={name,set:1,type,rounds:n('rounds'),minutes:n('minutes')}; if(Object.keys(loads).length) l.loads=loads; logs.push(l); }
  else { if($(`#${id}-done`)?.checked) logs.push({name,set:1,type,note:'Completed'}); } }); return logs; }
function saveSessionEditor(){ const date=$('#psDate').value, time=$('#psTime').value||'12:00', day=$('#psDay').value, duration=Math.max(1,+$('#psDuration').value||30), mode=$('#psMode').value, notes=$('#psNotes').value.trim(); if(!date){ alert('Pick a date.'); return; } const when=new Date(`${date}T${time}:00`); if(isNaN(when)){ alert('That date or time is not valid.'); return; } const logs=collectSessionLogs(); const h=getState().history; const entry={id:editingSessionId||uid(),date:when.toISOString(),day,title:PROGRAM[day].title,duration,logs,summary:notes?(editingSessionId?notes:`Logged afterward: ${notes}`):(editingSessionId?'':'Logged afterward.'),mode,manual:true}; const idx=h.findIndex(x=>x.id===entry.id); if(idx>=0){ entry.manual=h[idx].manual||false; h[idx]=entry; } else h.unshift(entry); h.sort((a,b)=>new Date(b.date)-new Date(a.date)); STORE.set('hr30_history',h.slice(0,500)); updateWorkingWeightsFromLogs(logs); editingSessionId=null; closeModal(); trainView='history'; if(currentTab==='train') renderTrain(); else tab('today',false); }
function deleteSession(id){ if(!confirm('Delete this session from your history?')) return; STORE.set('hr30_history',getState().history.filter(x=>x.id!==id)); editingSessionId=null; closeModal(); trainView='history'; renderTrain(); }
function renderHistoryMarkup(){ const h=getState().history; return `<button class="secondary" style="width:100%;margin-bottom:12px" onclick="openPastSession()">＋ LOG A PAST SESSION</button><section class="card">${h.length?h.map(x=>`<div class="history-item"><div class="top"><div><h4>${x.day} — ${x.title}</h4><small>${new Date(x.date).toLocaleString()}</small></div><strong>${x.duration}m</strong></div><p class="note">${x.logs?.length||0} logged sets / blocks</p><div style="display:flex;gap:16px"><button class="ghost" onclick="viewHistory('${x.id||''}','${x.date}')">View details →</button><button class="ghost" onclick="openSessionEditor('${x.id||''}')">Edit</button></div></div>`).join(''):`<div class="empty">Complete your first workout and it will show up here.</div>`}</section>`; }
function viewHistory(id,date){ const h=getState().history; const x=h.find(v=>(id&&v.id===id)||v.date===date); if(!x) return; openModal(`<div class="close-row"><div><div class="kicker">WORKOUT HISTORY</div><h2>${x.day} — ${x.title}</h2><p class="note">${new Date(x.date).toLocaleString()} • ${x.duration} min</p></div><button class="icon-btn" onclick="closeModal()">×</button></div>${historyLogMarkup(x.logs||[])}${x.summary?`<div class="notice">${esc(x.summary)}</div>`:''}<button class="secondary" style="width:100%;margin-top:14px" onclick="openSessionEditor('${x.id||''}')">EDIT THIS SESSION</button>`); }
function historyLogMarkup(logs){ if(!logs.length) return '<div class="empty">No detailed sets were logged.</div>'; const groups={}; logs.forEach(l=>(groups[l.name]??=[]).push(l)); return Object.entries(groups).map(([name,arr])=>`<section class="card flat"><div class="kicker">${esc(name)}</div>${arr.map(l=>`<div class="weight-row"><span>${l.type==='circuit'?'Circuit':`Set ${l.set||'—'}`}</span><strong>${logDescription(l)}</strong></div>${l.loads?Object.entries(l.loads).map(([k,v])=>`<div class="weight-row"><span style="color:var(--muted)">${esc(k)}</span><strong>${v} lb</strong></div>`).join(''):''}`).join('')}</section>`).join(''); }
function logDescription(l){ if(l.type==='ruck') return `${l.weight||0} lb • ${l.minutes||0} min${l.distance?` • ${l.distance} mi`:''}`; if(l.type==='carry') return `${l.weight||0} lb • ${l.seconds||0} sec`; if(l.type==='timed') return `${l.seconds||0} sec`; if(l.type==='circuit') return `${l.rounds||0} rounds${l.minutes?` • ${l.minutes} min`:''}`; if(l.level) return `L${l.level}${l.levelName?` ${esc(l.levelName)}`:''} × ${l.reps||0}`; if(l.band) return `${esc(l.band)} band × ${l.reps||0}`; if(l.weight||l.reps) return `${l.weight||0} lb × ${l.reps||0}`; return esc(l.note||'Completed'); }

let learnFilter='all';
function learnEquipmentOf(name){ const gymNames=new Set([...Object.values(GYM_SUBS).map(v=>v.name).filter(Boolean),...Object.values(GYM_BLOCK_VARIANTS).flatMap(v=>Object.values(v).map(x=>x.name)).filter(Boolean)]); const bwNames=new Set(Object.values(EQUIPMENT_SUBS).flatMap(v=>[v.bodyweight?.name,v.bands?.name]).filter(Boolean)); if(gymNames.has(name)&&!['Bulgarian Split Squat','Single-Leg RDL'].includes(name)) return 'gym'; if(bwNames.has(name)||/Ladder|Band |Backpack|Prone|Table/.test(name)) return 'bodyweight'; return 'dumbbells'; }
function renderLearn(){ $('#main').innerHTML=`<section class="card"><div class="kicker">EXERCISE LIBRARY</div><h2>Learn the movement before you load it.</h2><p class="sub">Each guide has a form drawing and written cues that work offline, plus a short demo video you can play right here when you have a connection.</p><div class="notice"><strong>Start here:</strong> Hip Hinge → Goblet Squat → DB Romanian Deadlift. Training without dumbbells? The ladders (Squat, Hinge, Row, Push-up) are your main lifts. At a gym, start with Back Squat → Trap-Bar Deadlift → Bench Press.</div></section><div class="learn-search-wrap"><input id="learnSearch" class="learn-search" placeholder="Search exercises…" oninput="filterLearn(this.value)"><div class="segmented four" style="margin:10px 0 0">${[['all','All'],['dumbbells','Dumbbells'],['bodyweight','Bodyweight'],['gym','Gym']].map(([k,l])=>`<button class="${learnFilter===k?'active':''}" onclick="learnFilter='${k}';filterLearn($('#learnSearch').value)">${l}</button>`).join('')}</div></div><div id="learnList">${learnCards('')}</div>`; }
function learnCards(query=''){ const q=query.trim().toLowerCase(); const names=LEARN_ORDER.filter(n=>(learnFilter==='all'||learnEquipmentOf(n)===learnFilter||(learnFilter!=='bodyweight'&&['Warm-up','Shoulder Prep','Dynamic Warm-up','Mobility','Plank','Push-Ups','Hip Hinge','Step-Up','Reverse Lunge','Suitcase Carry','Farmer Carry','Bulgarian Split Squat','Single-Leg RDL'].includes(n)))&&(!q||n.toLowerCase().includes(q)||(guideFor(n).why||'').toLowerCase().includes(q))); return names.length?names.map((name,i)=>{const g=guideFor(name),video=DEMO_VIDEOS[name]?'<span class="tag">▶ Video demo</span>':'<span class="tag">Written guide</span>',drawing=FORM_POSES[name]?'<span class="tag">Form drawing</span>':''; return `<section class="card learn-card" onclick='showExerciseGuide(${JSON.stringify(name)},"learn")'><div class="learn-card-top"><div class="exercise-num">${i+1}</div><div><h3>${esc(name)}</h3><p class="sub">${esc(g.subtitle||'')}</p></div></div><div class="tag-row">${video}${drawing}<span class="tag">Form cues</span><span class="tag">Common mistakes</span></div><button class="ghost learn-open">Open guide →</button></section>`;}).join(''):'<div class="empty">No exercises match that search.</div>'; }
function filterLearn(v){ $('#learnList').innerHTML=learnCards(v); $$('.learn-search-wrap .segmented button').forEach(b=>b.classList.toggle('active',b.textContent.toLowerCase()===(learnFilter==='all'?'all':learnFilter))); }
...
const FORM_POSES = {
  ...
  "Back Squat": { labels:["TOP","BOTTOM"], A:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[80,110],ankleF:[80,136],footF:[93,138],elbow:[96,40],hand:[92,30],bar:[[80,30]]},
                 B:{head:[92,48],neck:[88,58],hip:[66,100],kneeF:[96,108],ankleF:[86,136],footF:[99,138],elbow:[104,66],hand:[100,58],bar:[[88,58]]}, note:"Bar on the upper back. Sit between the hips; chest stays up." },
  "Front Squat": { labels:["TOP","BOTTOM"], A:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[80,110],ankleF:[80,136],footF:[93,138],elbow:[100,38],hand:[86,34],bar:[[84,34]]},
                 B:{head:[86,50],neck:[84,60],hip:[68,100],kneeF:[98,108],ankleF:[86,136],footF:[99,138],elbow:[106,66],hand:[90,62],bar:[[88,62]]}, note:"Bar rides on the front of the shoulders. Elbows high, torso upright." },
  "Trap-Bar Deadlift": { labels:["SET","STAND"], A:{head:[104,60],neck:[98,68],hip:[62,94],kneeF:[76,118],ankleF:[80,136],footF:[93,138],elbow:[104,94],hand:[104,120],bar:[[100,124]]},
                 B:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[80,110],ankleF:[80,136],footF:[93,138],elbow:[84,56],hand:[86,82],bar:[[86,86]]}, note:"Flat back at the start, shoulders over the handles. Stand up through the whole foot." },
  "Barbell RDL": { labels:["START","HINGE"], A:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[80,110],ankleF:[80,136],footF:[93,138],elbow:[84,56],hand:[86,82],bar:[[86,86]]},
                 B:{head:[122,52],neck:[113,58],hip:[68,90],kneeF:[76,114],ankleF:[80,136],footF:[93,138],elbow:[113,80],hand:[113,106],bar:[[110,110]]}, note:"The bar drags down the thighs. Hips back until the hamstrings pull, back flat." },
  "Barbell Bench Press": { labels:["LOCKOUT","CHEST"], box:[10,112,90,10], A:{head:[18,104],neck:[28,104],hip:[80,106],kneeF:[100,118],ankleF:[112,136],footF:[122,138],elbow:[36,78],hand:[38,50],bar:[[38,46]]},
                 B:{head:[18,104],neck:[28,104],hip:[80,106],kneeF:[100,118],ankleF:[112,136],footF:[122,138],elbow:[48,100],hand:[42,80],bar:[[42,76]]}, note:"Safeties set first. Bar touches the lower chest; wrists stacked over elbows." },
  "Incline DB Bench Press": { labels:["PRESS","LOWER"], box:[8,100,60,12], A:{head:[30,66],neck:[38,74],hip:[76,104],kneeF:[100,116],ankleF:[112,136],footF:[122,138],elbow:[52,50],hand:[56,26],db:[[56,22]]},
                 B:{head:[30,66],neck:[38,74],hip:[76,104],kneeF:[100,116],ankleF:[112,136],footF:[122,138],elbow:[60,80],hand:[54,58],db:[[54,54]]}, note:"Low incline, about 30°. Elbows under the wrists; press up and slightly in." },
  "Overhead Press": { labels:["RACK","LOCKOUT"], A:{head:[80,42],neck:[80,54],hip:[80,96],kneeF:[80,118],ankleF:[80,136],footF:[93,138],elbow:[92,68],hand:[86,50],bar:[[86,48]]},
                 B:{head:[80,44],neck:[80,56],hip:[80,96],kneeF:[80,118],ankleF:[80,136],footF:[93,138],elbow:[84,34],hand:[82,14],bar:[[82,12]]}, note:"Glutes tight so the low back doesn't arch. Bar goes straight up; head comes through at the top." },
  "Cable Row": { labels:["REACH","PULL"], box:[120,70,30,68], A:{head:[62,40],neck:[64,52],hip:[66,100],kneeF:[100,96],ankleF:[118,110],footF:[120,98],elbow:[92,66],hand:[116,78],cable:[[116,78,150,78]]},
                 B:{head:[62,40],neck:[64,52],hip:[66,100],kneeF:[100,96],ankleF:[118,110],footF:[120,98],elbow:[56,84],hand:[74,88],cable:[[74,88,150,78]]}, note:"Torso stays tall and still. Pull the handle to the lower ribs and squeeze." },
  "Chest-Supported Row": { labels:["HANG","ROW"], box:[20,96,80,10], A:{head:[112,46],neck:[104,54],hip:[60,88],kneeF:[44,110],ankleF:[30,136],footF:[22,138],elbow:[100,82],hand:[100,110],db:[[100,114]]},
                 B:{head:[112,46],neck:[104,54],hip:[60,88],kneeF:[44,110],ankleF:[30,136],footF:[22,138],elbow:[84,70],hand:[98,86],db:[[98,90]]}, note:"Chest glued to the pad. Elbows toward the hips; the back can't cheat." },
  "Lat Pulldown": { labels:["REACH","PULL"], box:[40,104,60,34], A:{head:[74,44],neck:[74,56],hip:[70,102],kneeF:[98,104],ankleF:[100,136],footF:[112,138],elbow:[92,30],hand:[98,6],cable:[[98,6,98,-6]]},
                 B:{head:[74,44],neck:[74,56],hip:[70,102],kneeF:[98,104],ankleF:[100,136],footF:[112,138],elbow:[78,74],hand:[90,52],cable:[[90,52,96,-6]]}, note:"Chest up, lean back a few degrees. Pull the bar to the upper chest with the elbows." },
  "Face Pull": { labels:["REACH","PULL"], A:{head:[70,20],neck:[70,32],hip:[72,84],kneeF:[74,110],ankleF:[76,136],footF:[89,138],elbow:[92,36],hand:[114,30],cable:[[114,30,150,26]]},
                 B:{head:[70,20],neck:[70,32],hip:[72,84],kneeF:[74,110],ankleF:[76,136],footF:[89,138],elbow:[92,26],hand:[80,22],cable:[[80,22,150,26]]}, note:"Rope at face height. Elbows high and wide; hands finish beside the ears." },
  "EZ-Bar Curl": { labels:["DOWN","CURL"], A:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[80,110],ankleF:[80,136],footF:[93,138],elbow:[82,56],hand:[86,82],bar:[[88,84]]},
                 B:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[80,110],ankleF:[80,136],footF:[93,138],elbow:[82,56],hand:[100,44],bar:[[104,42]]}, note:"Elbows stay pinned. Curl to just past vertical, lower slowly." },
  "Calf Raise Machine": { labels:["STRETCH","UP"], box:[70,124,40,14], A:{head:[80,14],neck:[80,26],hip:[80,78],kneeF:[80,106],ankleF:[80,132],heel:[70,136],footF:[94,124],elbow:[96,36],hand:[94,20],bar:[[86,26]]},
                 B:{head:[80,2],neck:[80,14],hip:[80,66],kneeF:[80,94],ankleF:[80,118],heel:[70,120],footF:[94,124],elbow:[96,24],hand:[94,8],bar:[[86,14]]}, note:"Balls of the feet on the step. Heels drop below it, then rise all the way up." },
  "Trap-Bar Carry": { labels:["STEP","STEP"], A:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[92,110],ankleF:[98,136],footF:[110,138],kneeB:[70,110],ankleB:[62,136],footB:[56,138],elbow:[80,56],hand:[80,84],bar:[[80,88]]},
                 B:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[70,110],ankleF:[62,136],footF:[74,138],kneeB:[92,110],ankleB:[98,136],footB:[110,138],elbow:[80,56],hand:[80,84],bar:[[80,88]]}, note:"Deadlift it up, walk tall with short steps, hinge it back down." },
  "Sandbag Carry": { labels:["STEP","STEP"], A:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[92,110],ankleF:[98,136],footF:[110,138],kneeB:[70,110],ankleB:[62,136],footB:[56,138],elbow:[96,50],hand:[90,64],elbow2:[96,50],hand2:[90,64]},
                 B:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[70,110],ankleF:[62,136],footF:[74,138],kneeB:[92,110],ankleB:[98,136],footB:[110,138],elbow:[96,50],hand:[90,64],elbow2:[96,50],hand2:[90,64]}, note:"Bag hugged tight to the chest. Brace, breathe, short steps." },
  "Incline Treadmill Ruck": { labels:["STEP","STEP"], box:[10,116,140,6], A:{head:[80,14],neck:[80,26],hip:[76,78],kneeF:[92,106],ankleF:[96,132],footF:[108,132],kneeB:[66,106],ankleB:[58,130],footB:[52,132],elbow:[92,50],hand:[96,68]},
                 B:{head:[80,14],neck:[80,26],hip:[76,78],kneeF:[66,106],ankleF:[58,130],footF:[70,132],kneeB:[92,106],ankleB:[96,132],footB:[108,132],elbow:[92,50],hand:[96,68]}, note:"Pack riding high, hands off the rails, brisk uphill walk." },
  "Stair Climber Ruck": { labels:["STEP","STEP"], box:[86,90,30,48], A:{head:[70,14],neck:[70,26],hip:[68,78],kneeF:[92,84],ankleF:[96,104],footF:[110,104],kneeB:[62,108],ankleB:[60,132],footB:[72,134],elbow:[82,50],hand:[86,68]},
                 B:{head:[74,6],neck:[74,18],hip:[72,70],kneeF:[74,96],ankleF:[96,104],footF:[110,104],kneeB:[66,96],ankleB:[62,118],footB:[74,120],elbow:[86,42],hand:[90,60]}, note:"Whole foot on each step. Stand tall; if you need the rails, slow the machine down." },
  "Split Squat": { labels:["TOP","BOTTOM"], A:{head:[84,30],neck:[84,42],hip:[80,86],kneeF:[100,104],ankleF:[100,136],footF:[113,138],kneeB:[60,122],ankleB:[42,128],footB:[34,138],elbow:[84,68],hand:[84,92],db:[[84,96]]},
                 B:{head:[84,48],neck:[84,60],hip:[80,104],kneeF:[104,110],ankleF:[100,136],footF:[113,138],kneeB:[58,134],ankleB:[40,130],footB:[32,138],elbow:[84,86],hand:[84,110],db:[[84,114]]}, note:"Feet stay planted. Straight down, straight up, front knee over the toes." },
  "Bulgarian Split Squat": { labels:["TOP","BOTTOM"], box:[10,112,40,26], A:{head:[86,26],neck:[86,38],hip:[82,84],kneeF:[104,102],ankleF:[104,136],footF:[117,138],kneeB:[58,114],ankleB:[36,110],footB:[24,112],elbow:[86,66],hand:[86,90],db:[[86,94]]},
                 B:{head:[88,46],neck:[88,58],hip:[82,102],kneeF:[108,108],ankleF:[104,136],footF:[117,138],kneeB:[56,128],ankleB:[36,110],footB:[24,112],elbow:[88,86],hand:[88,110],db:[[88,114]]}, note:"Back foot rests on the bench, laces down. The front leg does the lifting." },
  "Staggered-Stance RDL": { labels:["STAND","HINGE"], A:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[80,110],ankleF:[80,136],footF:[93,138],kneeB:[70,110],ankleB:[62,130],footB:[58,138],elbow:[84,56],hand:[86,80],db:[[86,84]]},
                 B:{head:[122,52],neck:[113,58],hip:[68,90],kneeF:[76,114],ankleF:[80,136],footF:[93,138],kneeB:[62,118],ankleB:[56,130],footB:[52,138],elbow:[113,80],hand:[113,104],db:[[113,108]]}, note:"Back toe is a kickstand. Front hip pushes back, front leg does the work." },
  "Single-Leg RDL": { labels:["STAND","HINGE"], A:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[80,110],ankleF:[80,136],footF:[93,138],elbow:[84,56],hand:[86,80],db:[[86,84]]},
                 B:{head:[124,56],neck:[115,62],hip:[70,88],kneeF:[76,114],ankleF:[80,136],footF:[93,138],kneeB:[36,80],ankleB:[8,74],footB:[0,70],elbow:[113,84],hand:[113,108],db:[[113,112]]}, note:"Free leg and torso tip together like a seesaw. Hips stay square to the floor." },
  "Single-Arm Floor Press": { labels:["PRESS UP","LOWER"], A:{head:[18,128],neck:[28,126],hip:[80,128],kneeF:[100,106],ankleF:[112,136],footF:[122,138],elbow:[34,100],hand:[36,74],db:[[36,70]],elbow2:[44,126],hand2:[60,124]},
                 B:{head:[18,128],neck:[28,126],hip:[80,128],kneeF:[100,106],ankleF:[112,136],footF:[122,138],elbow:[46,124],hand:[42,100],db:[[42,96]],elbow2:[44,126],hand2:[60,124]}, note:"One dumbbell. The free hand rests on your stomach. Hips and shoulders stay flat." },
  "Gorilla Row": { labels:["BRACE","ROW"], A:{head:[122,58],neck:[113,64],hip:[68,92],kneeF:[76,116],ankleF:[80,136],footF:[93,138],elbow:[112,92],hand:[110,124],db:[[110,132],[132,132]],elbow2:[126,92],hand2:[132,124]},
                 B:{head:[122,58],neck:[113,64],hip:[68,92],kneeF:[76,116],ankleF:[80,136],footF:[93,138],elbow:[94,74],hand:[108,92],db:[[108,96],[132,132]],elbow2:[126,92],hand2:[132,124]}, note:"Hinged over two dumbbells. One rows to the hip while the other holds the floor." },
  "Renegade Row": { labels:["PLANK","ROW"], A:{head:[132,92],neck:[122,98],hip:[74,108],kneeF:[50,114],ankleF:[26,122],footF:[20,138],elbow:[124,118],hand:[126,134],db:[[126,136],[110,136]],elbow2:[112,118],hand2:[110,134]},
                 B:{head:[132,92],neck:[122,98],hip:[74,108],kneeF:[50,114],ankleF:[26,122],footF:[20,138],elbow:[124,118],hand:[126,134],db:[[126,136],[112,100]],elbow2:[118,100],hand2:[112,104]}, note:"A push-up position on the dumbbells. Row one to the ribs without letting the hips twist." },
  "Rack Carry": { labels:["STEP","STEP"], A:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[92,110],ankleF:[98,136],footF:[110,138],kneeB:[70,110],ankleB:[62,136],footB:[56,138],elbow:[92,48],hand:[86,34],db:[[86,30]],elbow2:[70,48],hand2:[74,34],db2:[[74,30]]},
                 B:{head:[80,18],neck:[80,30],hip:[80,82],kneeF:[70,110],ankleF:[62,136],footF:[74,138],kneeB:[92,110],ankleB:[98,136],footB:[110,138],elbow:[92,48],hand:[86,34],db:[[86,30]],elbow2:[70,48],hand2:[74,34],db2:[[74,30]]}, note:"Dumbbells rest on the shoulders, elbows forward. Tall, braced, short steps." },
...
function poseFrame(f, dim){
  ...
  const BAR=(arr)=>(arr||[]).map(([x,y])=>`<rect class="db" x="${x-24}" y="${y-2}" width="48" height="4" rx="2"/><rect class="db" x="${x-28}" y="${y-8}" width="6" height="16" rx="1"/><rect class="db" x="${x+22}" y="${y-8}" width="6" height="16" rx="1"/>`).join('');
  const CABLE=(arr)=>(arr||[]).map(([x1,y1,x2,y2])=>`<line class="limb back" x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke-dasharray="3 3"/>`).join('');
  return `<g class="fig ${dim?'dim':''}">${P([f.kneeB&&f.hip,f.kneeB,f.ankleB,f.footB],'limb back')}${L(f.neck,f.elbow2,'limb back')}${L(f.elbow2,f.hand2,'limb back')}${DB(f.db2)}`+
         `${L(f.neck,f.hip,'torso')}${P([f.hip,f.kneeF,f.ankleF,f.footF],'limb')}${f.heel?L(f.heel,f.ankleF,'limb')+L(f.heel,f.footF,'limb'):''}${L(f.neck,f.elbow,'limb')}${L(f.elbow,f.hand,'limb')}${DB(f.db)}${BAR(f.bar)}${CABLE(f.cable)}<circle class="head" cx="${f.head[0]}" cy="${f.head[1]}" r="8"/></g>`;
}
...
function workingWeightExercises(){ const names=[], mode=baseEquipment(), blk=currentBlock(); Object.keys(PROGRAM).forEach(d=>{ PROGRAM[d].exercises.map(e=>applyBlock(substituteExercise(e,mode),blk,mode)).forEach(e=>{ if(['strength','carry'].includes(e.type)&&!e.bodyweightOnly&&!names.includes(e.name)) names.push(e.name); if(e.type==='circuit') circuitLoadedItems(e).forEach(it=>{ const k=circuitLoadKey(it.name); if(!names.includes(k)) names.push(k); }); }); }); getState().history.forEach(h=>(h.logs||[]).forEach(l=>{ if(['strength','carry'].includes(l.type)&&(l.weight||l.level||l.band)&&!names.includes(l.name)&&exerciseDef(l.name)) names.push(l.name); })); return names; }
...
function renderProgress(){
  ...
  $('#main').innerHTML=`<div class="section-title"><h3>Progress</h3><small>make improvement visible</small></div>
    ...
    <section class="card"><div class="section-title" style="margin:0 0 8px"><h3>Current loads &amp; levels</h3><small>${modeLabel(baseEquipment())} • tap to edit</small></div>${names.map(n=>{const d=exerciseDef(n);return `<div class="weight-row"><div><strong>${esc(n)}</strong><small>${n.startsWith(CIRCUIT_PREFIX)?'Friday circuit load':d?.ladder?'current ladder level':d?.band?'band strength':d?.barbell?'total bar weight':/Carry/.test(n)?'carried load':d?.subFor&&baseEquipment()==='gym'?'working load':'working dumbbell load'}</small></div><button class="secondary" onclick='editWorkingWeight(${JSON.stringify(n)})'>${loadLabel(n,s.workingWeights[wwKey(n,exerciseDef(n))])}</button></div>`;}).join('')}</section>
...
let weaponView='bow', pendingRiflePhotos=[];
function openWeaponProfile(view){ if(view) weaponView=view; else { const nh=nextHunt(); weaponView=(nh&&['rifle','muzzleloader'].includes(nh.weapon))?'rifle':(weaponView||'bow'); } if(weaponView==='rifle') openRifleProfile(); else openBowProfile(); }
function weaponSwitch(){ return `<div class="segmented" style="margin:0 0 14px"><button class="${weaponView==='bow'?'active':''}" onclick="openWeaponProfile('bow')">${ICONS.bow.replace('<svg','<svg class="seg-icon"')} Bow</button><button class="${weaponView==='rifle'?'active':''}" onclick="openWeaponProfile('rifle')">${ICONS.rifle.replace('<svg','<svg class="seg-icon"')} Rifle</button></div>`; }
function openRifleProfile(){
  const r=getState().rifleProfile||{}; pendingRiflePhotos=[];
  openModal(`<div class="close-row"><div><div class="kicker">WEAPON PROFILE</div><h2>Your rifle setup</h2></div><button class="icon-btn" onclick="closeModal()">×</button></div>${weaponSwitch()}
    <div class="field"><label>Photos of your rifle (up to 5)</label><input type="file" accept="image/*" multiple onchange="handleRiflePhotos(this.files)"></div><div id="riflePhotoStrip" class="frame-strip"></div><p class="note">Best results: the whole rifle from the side, the receiver or barrel stamp, the scope turret caps, and an ammo box.</p><div class="ai-status" id="rifleAiStatus">${getState().settings.aiEndpoint?'Add photos, then scan. The AI fills empty fields and only replaces what it can actually read.':'AI scan needs the secure endpoint in Settings. You can still fill this in by hand.'}</div><button class="secondary" id="rifleScanBtn" style="width:100%;margin-top:8px" onclick="scanRiflePhotos()">SCAN MY RIFLE WITH AI</button>
    <div class="set-grid"><div class="field"><label>Rifle (make / model)</label><input id="rifleName" value="${esc(r.rifle||'')}" placeholder="e.g. Tikka T3x"></div><div class="field"><label>Caliber</label><input id="rifleCaliber" value="${esc(r.caliber||'')}" placeholder="e.g. 6.5 Creedmoor"></div></div>
    <div class="set-grid"><div class="field"><label>Scope</label><input id="rifleScope" value="${esc(r.scope||'')}" placeholder="e.g. Leupold VX-3HD 3.5-10×40"></div><div class="field"><label>Zeroed at (yd)</label><input id="rifleZero" inputmode="numeric" value="${r.zeroYd||''}" placeholder="200"></div></div>
    <div class="field"><label>Ammo (brand / bullet / grain)</label><input id="rifleAmmo" value="${esc(r.ammo||'')}" placeholder="e.g. Hornady Precision Hunter 143 gr ELD-X"></div>
    <div class="set-grid"><div class="field"><label>Handedness</label><select id="rifleHand"><option value="right" ${r.handedness!=='left'?'selected':''}>Right-handed</option><option value="left" ${r.handedness==='left'?'selected':''}>Left-handed</option></select></div><div class="field"><label>Accessories</label><input id="rifleAccessories" value="${esc(r.accessories||'')}" placeholder="Sling, bipod, suppressor"></div></div>
    <div class="field"><label>Setup notes</label><textarea id="rifleNotes" rows="3">${esc(r.notes||'')}</textarea></div>
    <button class="primary" onclick="saveRifleProfile()">SAVE RIFLE PROFILE</button>`);
  (async()=>{ const keys=(await PhotoStore.keys()).filter(k=>String(k).startsWith('rifle-photo')).sort(); const arr=[]; for(const k of keys){ const src=await PhotoStore.get(k); if(src) arr.push(src); } if(arr.length){ pendingRiflePhotos=arr; renderRifleStrip(); } })();
}
function renderRifleStrip(){ const el=$('#riflePhotoStrip'); if(el) el.innerHTML=pendingRiflePhotos.map(src=>`<img src="${src}" alt="">`).join(''); }
async function handleRiflePhotos(files){ const list=[...(files||[])].slice(0,5); if(!list.length) return; pendingRiflePhotos=[]; for(const f of list) pendingRiflePhotos.push(await compressImage(f,1024,.8)); renderRifleStrip(); $('#rifleAiStatus').textContent=getState().settings.aiEndpoint?`${pendingRiflePhotos.length} photo${pendingRiflePhotos.length===1?'':'s'} ready. Tap Scan my rifle.`:'Photos attached. AI scan needs the secure endpoint in Settings.'; }
async function scanRiflePhotos(){ if(!pendingRiflePhotos.length){ alert('Add a photo of your rifle first.'); return; } const d=await callAi('archery',{kind:'rifle',imageDataUrls:pendingRiflePhotos},$('#rifleAiStatus'),'#rifleScanBtn','SCANNING…'); if(!d) return; const readable=new Set(Array.isArray(d.readable_fields)?d.readable_fields:[]); const filler=v=>!v||/^(none|unknown|not visible|n\/a|none visible)/i.test(String(v).trim()); const filled=[],updated=[],kept=[]; const put=(id,key,val,label)=>{ const el=$(id); if(!el||filler(val)) return; const cur=el.value.trim(); if(!cur){ el.value=String(val).trim(); filled.push(label); } else if(readable.has(key)&&cur!==String(val).trim()){ el.value=String(val).trim(); updated.push(label); } else if(cur!==String(val).trim()) kept.push(label); }; put('#rifleName','rifle',[d.brand,d.model].filter(x=>!filler(x)).join(' '),'Rifle'); put('#rifleCaliber','caliber',d.caliber,'Caliber'); put('#rifleScope','scope',d.scope,'Scope'); put('#rifleAmmo','ammo',d.ammo,'Ammo'); put('#rifleAccessories','accessories',d.accessories,'Accessories'); if(d.confidence==='high'&&['left','right'].includes(d.handedness)) $('#rifleHand').value=d.handedness; const n=$('#rifleNotes'); if(d.other&&!filler(d.other)) n.value=(n.value?n.value+'\n':'')+`AI scan (${d.confidence} confidence): ${d.other}`; const parts=[]; if(filled.length) parts.push('Filled: '+filled.join(', ')); if(updated.length) parts.push('Updated from a label: '+updated.join(', ')); if(kept.length) parts.push('Kept yours: '+kept.join(', ')); if(!parts.length) parts.push('Nothing changed in your fields'); $('#rifleAiStatus').textContent=`${parts.join('. ')}. ${d.notes||''} Check it, then save.`; }
async function saveRifleProfile(){ { const keys=(await PhotoStore.keys()).filter(k=>String(k).startsWith('rifle-photo')); for(const k of keys) await PhotoStore.del(k); for(let i=0;i<pendingRiflePhotos.length;i++) await PhotoStore.set(i===0?'rifle-photo':`rifle-photo-${i+1}`,pendingRiflePhotos[i]); } STORE.set('hr30_rifleProfile',{rifle:$('#rifleName').value.trim(),caliber:$('#rifleCaliber').value.trim(),scope:$('#rifleScope').value.trim(),zeroYd:+$('#rifleZero').value||0,ammo:$('#rifleAmmo').value.trim(),handedness:$('#rifleHand').value,accessories:$('#rifleAccessories').value.trim(),notes:$('#rifleNotes').value.trim()}); ensureRifleChecklist(); closeModal(); renderToday(); }
function openBowProfile(){
  const b=getState().bowProfile; pendingBowPhotos=[];
  openModal(`<div class="close-row"><div><div class="kicker">WEAPON PROFILE</div><h2>Your bow setup</h2></div><button class="icon-btn" onclick="closeModal()">×</button></div>${weaponSwitch()}
...
function sessionRemaining(){ if(!active) return 0; return Math.max(0, active.workout.duration*60 - Math.floor((Date.now()-active.started)/1000)); }
function tickTimers(){ if(!active) return; const el=$('#workoutTimer'); if(el) el.textContent=fmt(sessionRemaining()); const bt=$('#blockTimer'); if(bt&&active.block){ const left=blockLeft(); bt.textContent=fmt(left); const tr=$('#blockTrack'); if(tr) tr.style.width=`${Math.max(0,left/active.block.total*100)}%`; if(left<=0&&!active.block.done){ active.block.done=true; Alerts.go(); bt.closest('.block-timer')?.classList.add('go'); const b=$('#blockBtn'); if(b) b.textContent='DONE'; } else if(left>0&&left<=3&&active.block.lastTick!==left){ active.block.lastTick=left; Alerts.tick(); } } }
function startWorkout(day){ Alerts.unlock(); const w=effectiveWorkout(day); active={day,workout:w,index:0,set:1,logs:[],started:Date.now(),historyStack:[],mode:'workout',block:null}; openModal(workoutModal()); clearInterval(workoutTimer); workoutTimer=setInterval(tickTimers,500); }
function blockSeconds(e){ if(e.type==='time'||e.type==='circuit'||e.type==='ruck') return (e.minutes||0)*60; if(e.type==='timed') return e.seconds||0; return 0; }
function blockLeft(){ const b=active.block; if(!b) return 0; const elapsed=b.running?b.elapsedBefore+(Date.now()-b.startedAt)/1000:b.elapsedBefore; return Math.max(0, Math.round(b.total-elapsed)); }
function blockTimerMarkup(e){ const total=blockSeconds(e); if(!total) return ''; const b=active.block&&active.block.exIndex===active.index&&active.block.set===active.set?active.block:null; const left=b?blockLeft():total; return `<div class="block-timer ${b&&b.done?'go':''}"><div class="kicker">${e.type==='timed'?'Hold':'Block'} timer</div><div class="timer" id="blockTimer">${fmt(left)}</div><div class="rest-track"><span id="blockTrack" style="width:${Math.round(left/total*100)}%"></span></div><div class="action-row" style="margin-top:10px"><button class="secondary" onclick="resetBlock()">RESET</button><button class="primary" style="margin-top:0" id="blockBtn" onclick="toggleBlock()">${b?(b.done?'DONE':b.running?'PAUSE':'RESUME'):'START'}</button></div></div>`; }
function toggleBlock(){ Alerts.unlock(); const e=currentEx(), total=blockSeconds(e); if(!total) return; let b=active.block; if(!b||b.exIndex!==active.index||b.set!==active.set){ b=active.block={exIndex:active.index,set:active.set,total,elapsedBefore:0,startedAt:Date.now(),running:true,done:false,lastTick:null}; } else if(b.done){ return; } else if(b.running){ b.elapsedBefore+=(Date.now()-b.startedAt)/1000; b.running=false; } else { b.startedAt=Date.now(); b.running=true; } const btn=$('#blockBtn'); if(btn) btn.textContent=b.running?'PAUSE':'RESUME'; tickTimers(); }
function resetBlock(){ active.block=null; rerenderWorkout(); }
...
function suggestedLevel(name,max){ const inSession=[...active.logs].reverse().find(l=>l.name===name&&l.level); if(inSession)return inSession.level; for(const h of getState().history){const f=[...(h.logs||[])].reverse().find(l=>l.name===name&&l.level);if(f)return Math.min(max,f.level);} const w=getState().workingWeights[name+'::level']; return Math.min(max,Number(w)||1); }
...
function snapshot(){ return {index:active.index,set:active.set,logs:JSON.parse(JSON.stringify(active.logs)),mode:active.mode}; }
function restoreSnapshot(s){ active.index=s.index;active.set=s.set;active.logs=JSON.parse(JSON.stringify(s.logs));active.mode=s.mode;active.block=null; }
function workoutInputs(e){ const weight=suggestedWeight(e.name); if(e.type==='strength'&&e.ladder){ const lvl=suggestedLevel(e.name,e.ladder.length); return `<div class="field"><label>Level</label><select id="levelInput">${e.ladder.map((n,i)=>`<option value="${i+1}" ${i+1===lvl?'selected':''}>L${i+1} · ${esc(n)}</option>`).join('')}</select></div><div class="field"><label>Reps</label><input id="repsInput" inputmode="numeric" value="${e.min||''}"></div><p class="note">Set ${active.set} of ${e.sets} • hit ${e.max} clean reps every set to move up a level</p>`; } if(e.type==='strength'&&e.band){ const b=suggestedBand(e.name); return `<div class="set-grid"><div class="field"><label>Band</label><select id="bandInput">${BAND_LEVELS.map(n=>`<option ${n===b?'selected':''}>${n}</option>`).join('')}</select></div><div class="field"><label>Reps</label><input id="repsInput" inputmode="numeric" value="${e.min||''}"></div></div><p class="note">Set ${active.set} of ${e.sets}</p>`; } if(e.type==='strength'&&e.subFor&&!e.ladder&&!e.band&&!e.barbell&&active?.workout?.mode==='bodyweight'){ return `<div class="set-grid"><div class="field"><label>Pack load (lb, optional)</label><input id="weightInput" inputmode="decimal" value="${weight}"></div><div class="field"><label>Reps</label><input id="repsInput" inputmode="numeric" value="${e.min||''}"></div></div><p class="note">Set ${active.set} of ${e.sets}</p>`; } if(e.type==='strength'&&e.bodyweightOnly)return `<div class="field"><label>Reps</label><input id="repsInput" inputmode="numeric" value="${e.min||''}"></div><p class="note">Set ${active.set} of ${e.sets} • elevate your feet once ${e.max} is easy</p>`; if(e.type==='strength')return `<div class="set-grid"><div class="field"><label>${e.barbell?'Bar weight (lb, bar included)':'Weight (lb)'}</label><input id="weightInput" inputmode="decimal" value="${weight}" placeholder="${e.barbell?'45 = empty bar':''}"></div><div class="field"><label>Reps</label><input id="repsInput" inputmode="numeric" value="${e.min||''}"></div></div><p class="note">Set ${active.set} of ${e.sets}</p>`; if(e.type==='carry')return `<div class="set-grid"><div class="field"><label>Load (lb)</label><input id="weightInput" inputmode="decimal" value="${weight}"></div><div class="field"><label>Seconds</label><input id="secondsInput" inputmode="numeric" value="${e.seconds||40}"></div></div><p class="note">Carry ${active.set} of ${e.sets}</p>`; if(e.type==='timed')return `<div class="field"><label>Seconds</label><input id="secondsInput" inputmode="numeric" value="${e.seconds||45}"></div><p class="note">Set ${active.set} of ${e.sets}</p>`; if(e.type==='ruck')return `<div class="set-grid"><div class="field"><label>Load (lb)</label><input id="weightInput" inputmode="decimal" value="${getState().preferredRuck}"></div><div class="field"><label>Minutes</label><input id="minutesInput" inputmode="numeric" value="${e.minutes||30}"></div><div class="field"><label>Distance (mi, optional)</label><input id="distanceInput" inputmode="decimal"></div></div>`; if(e.type==='circuit')return `<div class="set-grid"><div class="field"><label>Rounds completed</label><input id="roundsInput" inputmode="decimal"></div><div class="field"><label>Minutes</label><input id="minutesInput" inputmode="numeric" value="${e.minutes||20}"></div></div>${circuitLoadInputs(e,'circuitLoad')}<div class="field"><label>Notes</label><input id="noteInput" placeholder="Optional"></div>`; return `<div class="field"><label>Notes / result</label><input id="noteInput" placeholder="Optional"></div>`; }
function workoutModal(){ const e=currentEx(),back=active.historyStack.length?'BACK':'EXIT'; return `<div class="workout-header"><div><div class="kicker">${active.day}</div><h2 style="margin:4px 0">${active.workout.title}</h2></div><div class="timer" id="workoutTimer">${fmt(sessionRemaining())}</div></div><div class="current-ex"><div class="kicker">EXERCISE ${active.index+1} OF ${active.workout.exercises.length}</div><h3>${esc(e.name)}</h3><p class="sub">${esc(e.prescription)}</p>${notesMarkup(e.notes)}${e.safety&&SAFETY[e.safety]?`<div class="notice safety"><strong>Safety.</strong> ${SAFETY[e.safety]}</div>`:''}${blockTimerMarkup(e)}<button class="secondary" style="width:100%;margin-top:10px" onclick='showExerciseGuide(${JSON.stringify(e.name)},"workout")'>HOW TO DO THIS EXERCISE</button>${workoutInputs(e)}<div class="action-row"><button class="secondary" onclick="previousStep()">${back}</button><button class="primary" style="margin-top:0" onclick="completeSet()">COMPLETE</button></div></div><button class="ghost" style="margin-top:16px" onclick="finishWorkout(true)">Finish early</button>`; }
function completeSet(){ Alerts.unlock(); const prev=snapshot(),e=currentEx(),log={name:e.name,set:active.set,type:e.type}; if(active.block&&active.block.exIndex===active.index){ const used=Math.round(active.block.total-blockLeft()); if(used>0) log.timedSeconds=used; } active.block=null; if(e.subFor) log.subFor=e.subFor; if($('#levelInput')){log.level=+$('#levelInput').value||1;log.levelName=e.ladder?e.ladder[log.level-1]:'';} if($('#bandInput'))log.band=$('#bandInput').value; if($('#weightInput'))log.weight=+$('#weightInput').value||0;if($('#repsInput'))log.reps=+$('#repsInput').value||0;if($('#secondsInput'))log.seconds=+$('#secondsInput').value||0;if($('#minutesInput'))log.minutes=+$('#minutesInput').value||0;if($('#distanceInput'))log.distance=+$('#distanceInput').value||0;if($('#roundsInput'))log.rounds=+$('#roundsInput').value||0;if($('#noteInput'))log.note=$('#noteInput').value; if(e.type==='circuit'&&circuitLoadedItems(e).length){ const loads=readCircuitLoads('circuitLoad'); if(Object.keys(loads).length) log.loads=loads; } active.logs.push(log);active.historyStack.push(prev); const multi=e.sets&&active.set<e.sets;if(multi){active.set++;e.rest?startRest(e.rest):rerenderWorkout();}else{active.index++;active.set=1;if(active.index>=active.workout.exercises.length){finishWorkout(false);return;}e.rest?startRest(e.rest):rerenderWorkout();} }
function startRest(seconds){ active.mode='rest'; const endAt=Date.now()+seconds*1000; let lastShown=null; $('#modal').innerHTML=`<div class="close-row"><div><div class="kicker">REST</div><h2>Recover, then go.</h2></div><button class="icon-btn" onclick="skipRest()">→</button></div><div class="rest-box"><div class="kicker">Next · ${active.index<active.workout.exercises.length?esc(currentEx().name):'Finish'}</div><div class="timer" id="restTimer">${fmt(seconds)}</div><div class="rest-track"><span id="restTrack" style="width:100%"></span></div><p class="sub">Ticks at 3 · 2 · 1 · GO</p></div><div class="action-row"><button class="secondary" onclick="previousStep()">BACK</button><button class="primary" onclick="skipRest()">SKIP REST</button></div>`; clearInterval(restTimer); const tick=()=>{ const left=Math.max(0,Math.ceil((endAt-Date.now())/1000)); const el=$('#restTimer'); if(el) el.textContent=fmt(left); const tr=$('#restTrack'); if(tr) tr.style.width=`${Math.max(0,(endAt-Date.now())/(seconds*1000)*100)}%`; if(left!==lastShown){ lastShown=left; if(left>0&&left<=3) Alerts.tick(); } if(left<=0){ clearInterval(restTimer); Alerts.go(); const box=document.querySelector('.rest-box'); if(box) box.classList.add('go'); setTimeout(()=>{ if(active&&active.mode==='rest') skipRest(); },700); } }; restTimer=setInterval(tick,250); tick(); }
...
function updateWorkingWeightsFromLogs(logs){ const w=getState().workingWeights;logs.forEach(l=>{ if(l.level) w[l.name+'::level']=l.level; else if(l.band) w[l.name]=l.band; else if(['strength','carry'].includes(l.type)&&l.weight) w[l.name]=l.weight; else if(l.type==='circuit'&&l.loads) Object.entries(l.loads).forEach(([k,v])=>{ if(v) w[circuitLoadKey(k)]=v; }); });STORE.set('hr30_workingWeights',w); }
function finishWorkout(early){ if(!active)return;clearInterval(workoutTimer);clearInterval(restTimer);const duration=Math.max(1,Math.round((Date.now()-active.started)/60000)),summary=progressionSummary(active.logs),entry={id:uid(),date:new Date().toISOString(),day:active.day,title:active.workout.title,duration,logs:active.logs,summary,mode:active.workout.mode||'dumbbells'};saveHistory(entry);updateWorkingWeightsFromLogs(active.logs);active=null;closeModal();tab('today');setTimeout(()=>{ const c=document.querySelector('.done-card'); if(c) c.scrollIntoView({behavior:'smooth',block:'center'}); },150); }
function progressionSummary(logs){ if(active?.workout?.readinessAdj) return 'Adjusted for readiness. Loads held today.'; const groups={};logs.forEach(l=>{if(l.type==='strength'&&l.reps&&(l.weight||l.level||l.band||exerciseDef(l.name)?.bodyweightOnly))(groups[l.name]??=[]).push(l)});const recs=[];Object.entries(groups).forEach(([name,arr])=>{const e=exerciseDef(name,active?.workout?.mode);if(!(e?.max&&arr.length>=e.sets&&arr.every(x=>x.reps>=e.max)))return; if(e.ladder){ const lvl=Math.max(...arr.map(x=>x.level||1)); if(lvl<e.ladder.length) recs.push(`${name}: move up to L${lvl+1} (${e.ladder[lvl]}) next time`); else recs.push(`${name}: top of the ladder — add a loaded backpack or slow the tempo`); } else if(e.band){ const idx=BAND_LEVELS.indexOf(arr.at(-1).band||'Medium'); recs.push(idx<BAND_LEVELS.length-1?`${name}: try the ${BAND_LEVELS[idx+1].toLowerCase()} band next time`:`${name}: double up bands or slow the tempo`); } else if(e.bodyweightOnly) recs.push(`${name}: add 2 reps or elevate your feet next time`); else if(e.barbell) recs.push(`${name}: add 5–10 lb to the bar next time`); else recs.push(`${name}: consider +2.5–5 lb next time`);});return recs.join(' • '); }
function quickLogRecovery(){ saveHistory({id:uid(),date:new Date().toISOString(),day:'Sunday',title:'Recovery',duration:0,logs:[],summary:'Recovery day logged. Rest is obedience too.'});renderToday(); }
...
$('#modalBackdrop').addEventListener('click',e=>{ if(e.target.id==='modalBackdrop'&&!active&&!document.querySelector('.onboard'))closeModal(); });
...
if('serviceWorker' in navigator){
  ...
  // When an updated service worker takes control, reload once so the new version shows immediately.
  let hadController=!!navigator.serviceWorker.controller;
  navigator.serviceWorker.addEventListener('controllerchange',()=>{ if(hadController&&!active){ location.reload(); } hadController=true; });
}
renderToday();
