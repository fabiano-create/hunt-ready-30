...
async function analyzePendingMealPhoto(){ const endpoint=getState().settings.aiEndpoint; if(!pendingMealPhoto){alert('Add a meal photo first.');return;} if(!endpoint){ $('#aiStatus').textContent='AI scan is not connected. Add a secure nutrition-analysis endpoint in Settings. Do not put a private API key into this static app.'; return; } $('#aiStatus').textContent='Analyzing…'; try{ const res=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({imageDataUrl:pendingMealPhoto})}); if(!res.ok) throw new Error(`HTTP ${res.status}`); const d=await res.json(); if(d.name) $('#mealName').value=d.name; [['mealCalories','calories'],['mealProtein','protein'],['mealCarbs','carbs'],['mealFat','fat'],['mealFiber','fiber']].forEach(([id,k])=>{if(d[k]!=null)$('#'+id).value=d[k];}); if(d.notes) $('#mealNotes').value=d.notes; $('#aiStatus').textContent='AI estimate loaded. Review the numbers before saving.'; }catch(e){ $('#aiStatus').textContent=`AI analysis failed: ${e.message}`; } }
