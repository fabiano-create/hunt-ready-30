...
function renderProgress(){
  ...
  $('#main').innerHTML=`<div class="section-title"><h3>Progress</h3><small>make improvement visible</small></div>
    <section class="card"><div class="kicker">EXERCISE PROGRESSION</div><div class="field"><label>Exercise</label><select onchange="progressExerciseName=this.value;renderProgress()">${names.map(n=>`<option ${n===progressExerciseName?'selected':''}>${esc(n)}</option>`).join('')}</select></div><div class="progress-summary"><div class="stat"><strong>${last?loadLabel(progressExerciseName,exerciseDef(progressExerciseName)?.band?BAND_LEVELS[last.load-1]:last.load).replace(' lb',''):'—'}</strong><small>${exerciseDef(progressExerciseName)?.ladder?'last level':exerciseDef(progressExerciseName)?.band?'last band':'last load (lb)'}</small></div><div class="stat"><strong>${best?loadLabel(progressExerciseName,exerciseDef(progressExerciseName)?.band?BAND_LEVELS[best.load-1]:best.load).replace(' lb',''):'—'}</strong><small>${exerciseDef(progressExerciseName)?.ladder?'best level':exerciseDef(progressExerciseName)?.band?'best band':'best load (lb)'}</small></div><div class="stat"><strong>${data.length}</strong><small>sessions</small></div></div>${progressChart(data)}</section>
